...
The category $\FinOrd$ of finite ordinals and functions is a strict monoidal category where:
...
\begin{definition}
A {\bf \dag-category} ({\em read dagger-category}) is a category $\X$ equipped with a functor ${(\_)}^\dag:\X^\op\to\X$ ({\em read the dagger}).  This structure will arise both in our exposition of nondeterministic categorical semantics as well as for quantum computing.
...
This is a presentation for the prop of finite ordinal and functions $\FinOrd$.
...
The following result has almost surely been known for quite some time, but it is contained in \cite[\S 3.2]{zanasi}; where a much more fine grained presentation is given:
...
This is drawn as the string diagram:
TODO
...
There is a slighly more general notion than affine matrices, where there is no fixed origin:
...
Given composable  pair:
$$
n \xrightarrow{(M,a)} m \xrightarrow {(N,b)} k
$$
...
\item[Monoidal structure:]  The tensor product is given pointwise $(M,a)\otimes (N,b):=(M\oplus n, a\oplus b)$.  The tensor unit is the identity on $0$.
\end{description}
\end{definition}
...
This quotient of multiplicity from spans to relations in the case of finite sets, corresponds to applying the commutative semiring homorphism $\N\to \B$, where $\B$ is the Boolean semiring:
...
Given a field $k$, $\LinRel_{k}$ is generated by the generators and equations of the presentation of $\Mat_k$ as well as those of $\Mat_k^{\op}$ (drawn as the vertically flipped generators of $\Mat_k$) modulo the following equations for all $a \in k$, $a\neq 0$:
...
The $X$ stabilizers characterize the phase-free ZX-calculus (this has been known for quite some time to both the Italian and Oxford groups in the qubit case, see for example \cite[p. 8]{zanasi}):
