...
The generators of this presentation are interpreted in $\Mat(S)$ as follows:
...
In this section, we field of ``categorical quantum mechanics,'' intiated in the seminal paper  \cite{abramsky}.  The goal of this programme is to resynthesize finite-dimensional quantum theory using the theory of monoidal categories as well as their string diagrams.  The use of string diagrams in quantum theory actually goes back much further, for example in the work of Penrose \cite{penrosei,??}.  However, earlier work treated string diagrams as (often heuristic) tools used for calculation, rather than formal mathematical objects.  


First as a matter of notation if we fix a finite dimensional Hilbert space $\mathcal H$ with dimension $d\geq 2$. An element of $\mathcal H$ is called a {\bf qudit}.  A $\mathcal H$ is called a {\bf qubit} when $d=2$ and a {\bf quopit} when $d$ is an odd prime.
...
Oftentimes, we will fix a basis to work and call it the {\bf standard basis}.  In this case, we will denote the elements of the basis, in order, as  $\{| 0\rangle , \ldots, |d-1\rangle \}$.
...
Given two vectors $|\phi \rangle$ and $| \psi \rangle$ on the same finite-dimensional Hilbert space, the inner product is denoted by $\langle \phi | \psi \rangle$ and the outer product by $|\phi \rangle\langle \psi|$.  This notation for vectors and their adjoints is called bra-ket notation.
...
Special commutative \dag-Frobenius algebras in $\FHilb$ are in bijection with orthonormal bases. A basis $\{ |b_j\rangle \}_{j=0,\ldots, d-1}$ yields a Frobenius algebra where the spiders are of the form:
$$
...
A {\bf pure quantum state} is a vector $|\phi \rangle$ with norm 1, so that $\langle \phi | \phi \rangle =1$. These are regarded as the possible states of a quantum system which has been unexposed to the classical world.  The quantum evolution of pure quantum states is modelled by their postcomposition with unitary maps.  Unitary maps are precisely the linear maps which preserve quantum states, as they are norm-preserving.

However, this picture is not complete.  To actually do computations on classical systems, one has to measure things. 
In \cite{cpm}, Selinger gives a construction to produce categories of quantum channels from general \dag-compact closed categories. When applied to $\FHilb$, this construction adds discarding behaviour to quantum systems.  This will provide the necessary machinery to model measurement. 


We give a definition in terms of the $\CoPara$ construction for the sake of uniformity of this thesis:
\begin{definition}
\label{def:cpm}
...
\begin{example}
$\CPM(\FHilb, \bar{(\_ )})$ is the dagger compact closed category of density matrices between finite dimensional Hilbert spaces.
\end{example}

Density matrices model mixed quantum circuits.  The pure maps model a quantum system which has not been exposed to a classical system. The discarding map is interpreted as exposing the quantum system to the classical world,
...
\begin{tikzpicture}
	...
	\begin{pgfonlayer}{edgelayer}
		...
		\draw [in=75, out=60, looseness=3.00] (3) to (2);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
...
Take two bases in $\FHilb$ which we will denote respectively by white and grey Frobenius algebras. The bases are {\bf strongly complementary} when the corresponding Frobenius algebras interact to form a Hopf algebra whose antipode is equivalently any of the following maps:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
There is a normal form for the string diagrams generated by the components of a special commutative Frobenius algebra and its phase group.
...
TODO Stabilizer theory




Stabilizer fragment of the ZX-calculus
...
fragments of the ZH-calculus and H-boxes
