...
$$
\bigotimes_{j=0}^{n-1}{\cal X}^{a_j}
$$

Where note that:
$${\cal X}^a := \sum_{b=0}^{p-1} | b+a\rangle \langle b|$$
...
So it one can ask if the the fragment of the odd prime qudit ZX-calculus with these $\cal X$-gate phases has a similar relational semantics to the phase-free ZX-calculus.  The answer is yes, and this result is not contained in the literature to my knowledge:
...
$$
{\cal X}^x = \dfrac{1}{\sqrt{p}}\sum_{b=0}^{p-1} | b+x\rangle \langle b|
\ , \hspace*{.2cm}
{\cal Z}^z
:=
{\cal F}
...
\begin{lemma}\cite[Pg. 5]{gota}% https://arxiv.org/pdf/quant-ph/9802007.pdf
...
$$
\mathcal{C}_{\mathcal X}:= \sum_{j,k=0}^{p-1} |j,j+k \rangle \langle j,k |
\mathcal{M}_a := \sum_{j=0}^{p-1} |j\cdot a \rangle\langle j |
$$ 

Such that for qubits:
$$
\mathcal{S}:= \sum_{j=0}^{0} i^j |j\rangle\langle j |
$$
...
However, there is a crucial difference between the qubit and quopit cases:
...
In analogy to the ZX-calculus, the {\bf ZH-calculus} is presented by unphased $Z$ and $X$-spiders, the $\sf X$-gate and phased $H$-boxes.
