...
\begin{definition}
A {\bf \dag-category} ({\em read dagger-category}) is a category $\X$ equipped with a functor ${(\_)}^\dag:\X^\op\to\X$ (the dagger) which is.  This structure will arise both in our exposition of nondeterministic categorical semantics as well as for quantum computing.
...
Moreover, $\Hilb$ and $\Mat_\C$  are equivalent as $\dag$-symmetric monoidal categories.
\end{example}
...
Two diagrams in this prop whose components are connected have are equal, so we will use the following notation to refer to them:
...
This is a presentation for the prop of finite sets and relations $\FSets$.
This is a formal way to talk about the graph of a function between finite sets.

\end{example}

This elegant description of the symmetric monoidal category of finite sets motivates finding presentations for other well-known mathematical structures.  One way to combine monoidal theories is via pushout:
...
The unit and counit are interpreted as the unique matrices from $0 \to 1$ and $1\to 0$, respectively.
...
\begin{example}
Consider the symmetric monoidal theory generated by a commutative monoid $\zcirc$ in addition to this new generator and equation:
...
If functions produce unique outputs from inputs, spans and relations nondeterministicaly associate several inputs with several outputs.  To introduce these mathematical constructions, we first need to recall some basic facts about limits: 
...
The following Lemma justifies thinking of our running  examples of (finite) sets under the (co)product product as well as matrices, vector spaces and Hilbert spaces under the direct sum as symmetric monoidal categories:
...
We want to use coequalizers to ask for a requirements in which categories have notions of images and kernels:
...
\begin{lemma}
In a regular category, every map $f:X\to Y$ can be factorized into a regular epimorphism $e$ followed by a monomorphism $m$ up to unique isomorphism:
$$
\xymatrix{
X \ar@{->>}[rr]^{\hspace*{-.7cm}m_f} \ar[drr]_{f} &&{X/\ker{f}=:\co\im(f)} \ar@{>->}[d]^{e_f}\\
                                        &&Y
}
$$
\end{lemma}
...
In particular, we can truncate $\Rel$ to an ordinary category without taking any nasty quotients.



If one isn't interested in thinking hard about this abstract construction of internal relations, consider the following concrete example:
\begin{example}
$\Rel=\Rel(\Set)$ has:
...
This quotient of multiplicity from spans to relations in the case of finite sets, corresponds to applying the semiring homorphism $\N\to \B$, where $\B$ is the boolean semiring:

\begin{lemma}
$\Rel(\FSets)\cong \Mat(\B)$ so that it is presented by $\cb_\B$.
\end{lemma}

GIVE EXAMPLE OF APPLYING THIS QUOTIENT TO A STRING DIAGRAM
...
The category $\Aff\Mat_k$ is not a regular category, in fact it isn't even finitely complete; however, this is remedied by regarding the empty set as an affine space:
\begin{definition}
Let $\Aff\FVect_k$ denote the category of finite dimensional, possibly empty affine vector spaces.  $\Aff\FVect_k$ is a regular category, so we can define its category of internal relations; however, this is not a prop, so we remove one object:



The prop of {\bf affine relations} over $k$, $\Aff\Rel_{k}$ is the full subcategory of  $\Rel(\Aff\FVect_k)$  of nonempty affine subspaces.


Concretely, this is constructed in the same way as $\LinRel_k$ except a map $n\to m$ is instead a (possibly empty) affine subspace of $k^n\oplus k^m$.
\end{definition}


This admits a presentation (although, it was originally stated as a presentation for the concrete description, not as a full subcategory of such a category of internal relations):
...
So far, we have not talked about the Cartesian monoidal of categories of spans or internal relations.  Indeed, in both examples for which we have provided a presentation, the tensor product has been the coproduct/direct sum.

The essence of the Cartesian monoidal structure of categories of relations can be generalised to the following algebraic notion:
...
At least as far as the literature is concerned, the other direction is not so trvial.  We will first introduce the more copara construction, which freely adds an effect to every object in a way that is compatible with the tensor product:
...
In this section we review some basic results regarding internal categories.  By regarding categories as certain monads, this will allow us to compose categories via distributive law. 
...
Let us upack this a bit.  An small category has a {\em set} $\sf Ob$ of objects, and a set {\sf Ar} of maps.  There is a map ${\sf dom}:{\sf Ar}\to {\sf Ob}$ which picks out the domain of maps and another map  ${\sf codom}:{\sf Ar}\to {\sf Ob}$ picking out the codomain.  That is to say, a span of sets:
$$
S=\xymatrix{
...
In \cite{rosebrugh} they demonstrate that distributive laws of monads in  $\Span(\Set)$ correspond to strict factorization systems on the underlying internal category:  morphisms in the composite category $T;S$ of a distributive law $\lambda:S;T\to T;S$ can always be uniquely factored uniquely into the maps of $T$ followed by $S$.  
...
With the same objects.  Regard their corresponding pros $\bar{T_0}$ and $\bar{T_1}$ as monads in $\Span(\Mon)$ such that there is a a distributive $\lambda:\bar{T_0};\bar{T_1} \Rightarrow \bar{T_1};\bar{T_0}$.
...
{\sfa} is a presentation for $(\Span^\sim(\FinMonot^\op),+)$.
\end{lemma}
\begin{proof}
The idea is that a given a two cospans, the pushout of the cospan in the middle induces a mapping taking cospans to spans in $\FinMonot$, which is unique, up to unique isomorphism.  But we know that the skeleton of $\FinMonot$ is $\m$.  Therefore, it remains to check all of the critical pairs which occur in $\m^\op;\m$.  The special Frobenius laws are precisely the way to resolve these critical pairs and push the generators in $\m^\op$ past $\m$.
\end{proof}
...
This allows us to compose internal categories via distributive law modulo shared structure; the most basic shared structure, being the structure of the braids: 
...
With the same objects, where $\bar{T_2}$ is embeds as a symmetric monoidal category within both $\bar{T_0}$ and $\bar{T_3}$.  Regard both $\bar{T_0}$ and $\bar{T_0}$ as  $\bar{T_2}$-bimodules, where the left and right actions are given by lifting the maps in $\bar{T_2}$ along this embedding.
...
The simplest way to take distributive laws of props is to regard them as modules over free props:
...
The canonical left and right actions merely absorb the braids of $\P_n$ into the prop on the left and right.


Bialgebras arise in this way:
...
\end{tikzpicture}
$$
This is the prop $\scfa$ the prop for the free  special commutative Frobenius algebra, which we discussed earlier.
\end{example}
...
Actually, the original spider theorem was the spider theorem for non-special symmetric Frobenius algebras, first published in the PhD thesis of \cite{spider}; whereing it was proved by topological methods rather than using the machinery of distributive laws.
...
The compact closed structure of $\CPM(\X)$ is inhereted from the doubling of the compact closed structure of $\X$. And it is  compatible with the dagger that we just defined such that  $\CPM(\X)$ is \dag-compact closed.
\end{definition}
...
\begin{definition}
Given a category $\X$ and a class of idempotents $\mathcal I$ (so that $\forall e\in \mathcal I$, $e^2=e$), the {\bf Karoubi-envelope} at $\mathcal{I}$, ${\sf Split}_{\mathcal I}(\X)$ is the category with:
\begin{description}
...
In particular, when $\mathcal{I}$ is contains all idempotents in $\X$, call  ${\sf Split}(\X):={\sf Split}_{\mathcal I}(\X)$ {\bf the Karoubi envelope of $\X$}.
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=none] (76) at (24.925, -3.3) {};
		\node [style=Z] (77) at (24.925, -2.575) {};
		\node [style=none] (78) at (24.675, -2.325) {};
		\node [style=none] (79) at (24.2, -2.325) {};
		\node [style=none] (80) at (24.2, -3.325) {};
		\node [style=map] (151) at (24.925, -1.775) {$g$};
		\node [style=none] (152) at (24.95, -0.825) {};
		\node [style=none] (153) at (24.2, -0.075) {};
		\node [style=none] (154) at (24.2, 0.425) {};
		\node [style=Z] (160) at (24.2, -0.825) {};
		\node [style=none] (161) at (24.2, -0.825) {};
		\node [style=none] (162) at (24.95, -0.075) {};
		\node [style=none] (163) at (24.95, 0.425) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw [in=-75, out=135] (77) to (78.center);
		\draw [bend right=90, looseness=1.25] (78.center) to (79.center);
		...
		\draw [in=-90, out=90, looseness=0.75] (161.center) to (162.center);
		\draw [in=270, out=-90, looseness=1.25] (161.center) to (152.center);
		\draw [in=90, out=-105, looseness=1.25] (161.center) to (151);
		\draw (77) to (151);
	\end{pgfonlayer}
\end{tikzpicture}
$$
...
They are moreover {\bf strongly complementary} when the the bialgebras are Hopf algebras whoes antipode is equivalently any of the following maps:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
