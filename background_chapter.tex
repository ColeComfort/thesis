...
&
& (X\otimes Y)\otimes (X^*\otimes Y^*) \ar[d]^{1_{X\otimes Y} \otimes \sigma_{X^*,Y^*}}
\\
X^*\otimes( (X\otimes Y^*)\otimes Y)   \ar[u]^{1_{X^*}\otimes \alpha_{X,Y^*,Y}} &
&
&  (X\otimes Y)\otimes (Y^*\otimes X^*)
\\
X^*\otimes( (Y^*\otimes X)\otimes Y) \ar[u]^{1_{X^*}\otimes (\sigma_{Y^*,X}\otimes Y)} &
&
 &\\
X^*\otimes( Y^*\otimes (X\otimes Y))   \ar[u]^{1_{X^*} \otimes \alpha^{-1}_{Y^*,X,Y}} &
&  (X^*\otimes Y^*)\otimes (X\otimes Y)  \ar[ll]^{\ \ \ \ \ \alpha^{-1}_{X^*,Y^*,X\otimes Y}}
&  (Y^*\otimes X^*)\otimes (X\otimes Y) \ar[llluuuuuu]!<0.5ex,0.5ex>_{\epsilon_{X\otimes Y}} \ar[l]^{\sigma_{Y^*, X^*}\otimes 1_{X\otimes Y}}
}
$}
...
For the interested reader, he monoidal structures (or lack thereof) of categories of cospans and and spans of sets is elaborated on in great detail in \cite{bruni}.
...
The fact that internal relations is a strict bicategory means that we can regard is a an ordinary category; as, unlike it the case of spans, composition is strictly associative. Therefore, we don't have to quotient to obtain an ordinary category of relations.

Just as in the case of spans, relations is a \dag-compact closed category with respect to the Cartesian product where the \dag-functor is given by the converse.
...
By shifting to the category $\Aff\Mat_k+1$ where one freely adds the empty set as the initial object, so that there is a unique map $\emptyset \xrightarrow{?} k^n$, for all $n$,  we obtain an algebraic theory, and thus a regular category.  Therefore, we can take the category of internal relations:
...
{(f,S)\otimes (g;T) := ((f\otimes g);(1_{X\otimes S} \otimes \sigma_{W,T});\alpha_{X,S,T\otimes W};(1_X\otimes \alpha_{S,T,W}^{-1};(\sigma_{S,T}));\alpha_{Y,W,S\otimes T}^{-1} ,S\otimes T)} 
$
...
$$\mathbb{L} \otimes_{\mathbb J} \otimes \mathbb{R} \otimes_{\mathbb J}  \cdots \otimes_{\mathbb J} \mathbb{L} \otimes_{\mathbb J} \mathbb R  $$
 to one in 
$$\mathbb{L} \otimes_{\mathbb J} \otimes \mathbb{R}$$
then notice how $\mathbb J$ acts on $\mathbb L$ and $\mathbb R$ from both on the left and and on the right.  The one action is covariant and the other is contravariant.  So there is a priori no canonical way to slide all the maps in the various copies of $\mathbb J$ around and group them all together.  However, when $\mathbb J$ is a groupoid, a map $\phi:X\to Y$ in $\mathbb J$ induces another map $\phi^{-1}:Y\to X$, meaning that this is not a problem.
...
With the same objects, where $\bar{J}$ embeds as a strict symmetric monoidal category within both $\bar{R}$ and $\bar{L}$.  Regard both $\bar{R}$ and $\bar{R}$ as  $\bar{J}$-bimodules, where the left and right actions are given by lifting the maps in $\bar{J}$ along this embedding.
...
That is to say, where $\bar{R}$ and $\bar{L}$ are regarded as $(\bar{J}, \bar{J})$-bimodules where the 2-cell between them witnessing the distributive law lives in $\Mon$-$\Prof$.  Suppose moreover, that the  $\bar{L}$ and $\bar{R}$ are canonically strict monoidal subcategories of $\bar{L}\otimes_{\bar{J}} \bar{R}$
...
where $E_\lambda$ is the set of equations dictating the unique ways in which the generators of $\Sigma_R$ can be pushed past those of $\Sigma_L$ up to the maps in the maps in $\bar{J}$.
\end{lemma}
