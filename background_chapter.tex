...
In this section we review some category theory needed for this thesis.
...
\subsection{A note on proof nets}
...
\begin{example}
Consider the monoidal theory generated by a monoid on one object:
...
This is a presentation for the pro of finite ordinals and order perserving maps.
\end{example}
...
\end{tikzpicture}
$$
And generators for all elements $a,b \in k$ such that:
...
We see that $\FSet^\op$ embeds in this prop as well as in the prop $\Mat(S)$ for some semiring $S$.  The pushout of this diagram is presented by adding this generator and equations to the presentation of $\Mat(S)$.  In fact, this is a presentation for the prop of affine matrices over $S$!
\end{example}




There is another  more elegant way to compose presentations of monoidal and symmetric monoidal categories.  We will introduce this notion slightly later, because it requires a considerable amount of mathematical machinery to expose.
...
Where the semigroup and cosemigroup are daggers of each other and interact interact to form a semi-Frobenius algebra:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (0) at (0.75, 0.5) {};
...
\begin{lemma}
Given a two monoidal theories, and a distributive law between their corresponding multicoloured props, the induced prop has a monoidal theory

XXXXXXX
\end{lemma}

\begin{example}
TODO Cospans of sets and finite ordinals
\end{example}
...
\begin{lemma}
There is an equivalence of props $\cb\cong(\Span(\FinOrd),+)\cong (\Mat(\N),+)$.
\end{lemma}
...
The natural setting for quantum computing is the category $\FHilb$.  
Traditionally, calculations, and protocols in quantum computing are described algebraically, as I will proceed to sketch.

The states of the quantum system are regarded as normalized vectors $\C \to \mathcal{H}$. Using the bra-ket notation a state $\phi$ is denoted by  $|\phi\rangle$ with adjoint $\langle \phi|$. This notation interacts nicely with the contravariant order of composition.  The inner product of states $|\phi\rangle$ and  $|\psi\rangle$ is denoted by  $\langle \psi |\phi\rangle:= \langle \psi|\circ |\phi\rangle$ and the outer product by $|\phi\rangle\langle \psi | := |\phi\rangle\circ\langle \psi |$.
