...
\begin{example}
Both the category $\FSets$ of finite sets and functions and the category $\Sets$, of sets and functions is a monoidal category under both the product and under the coproduct.


%Finite ordinals and functions are a strict monoidal under the same tensor product.

Both the category $\FinMonot$ of finite sets and monotone functions and the category $\Monot$, of sets and monotone functions is a monoidal category under both the product and under the coproduct.
...
\begin{example}
The category $\FinOrd$ of  finite ordinals and functions is a strict monoidal category under the product and coproduct.

The category $\FinOrdMonot$ of finite ordinals and order-preserving maps is a strict monoidal category under the coproduct.

Given a semiring $S$, the category, $\Mat_S$, of matrices over $S$ is strict monoidal under the bilinear tensor product and the direct sum.
\end{example}
...
The strictification of a monoidal category has a particularly succinct presentation due to \cite{wilson}:
\begin{definition}
\label{def:proofnets}
...
Notice that the by flipping around the diagrams for the unitors and associators we get their inverses: 
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=none] (3) at (8, 3.5) {};
...
I will refer to the string diagrams in this category as proof nets for monoidal categories, because of their close relation to proof nets for linearly distributive categories.  Even in the case when we are already working in a strict monoidal category, it will still often be useful to use proof nets; for example, we can bundle up wires together and forgo having to forgo induction arguments.  Indeed, proof nets have been rediscovered in the setting of quantum circuits as the scalable ZX-calculus for precisely this reason \cite{szx} (although they have not discovered the units and counits).  We will discuss this further in Section \ref{sec:cqm}.
...
\begin{example}
$\FinOrdMonot$ is a skeletal category and is strict monoidal under the coproduct.  It is monoidally equivalent to $\FinMonot$ under the coproduct.


$\FinOrd$ is a skeletal category and strict monoidal. It is monoidally equivalent to $\FSets$ under the product and coproduct.


Given a semiring $R$, $\Mat_R$ is a skeletal a skeletal category and is strict monoidal. Moreover for a field $k$, $\Mat_k$ is monoidally equivalent to $\FVect_k$ under the bilinear tensor product and the direct sum.
...
Non-strict symmetric monoidal categories have essentially the same notion of proof nets as non-strict monoidal categories, except where the symmetry is internalized to untensoring, swapping and then tensoring:
...
For $\FHilb$ and $\FVect_k$, the compact closed structure is the same.  Given an orthonormal basis $\{ b_i \}_{i=0,\ldots, n-1}$ of a finite dimensional vector space $X$, with dual basis $\{b_i^* \}_{i=0,\ldots, n-1}$ of $X^*$, the unit and counit are given by the following linear maps:

$$
\eta_X = 1 \mapsto \sum_{i=0}^{n-1} b_i^* \otimes b_i \hspace*{1cm}\epsilon_X = \sum_{i=0}^{n-1}b_i\otimes b_i^* \mapsto \delta_{i,j}
$$
...
The unit and counit of $\Mat_R$ are essentially the same where $\epsilon_n=(1,\cdots, 1)$ and  $\eta_n=\epsilon_n^T$.
\end{example}


Similarly, their proof nets are essentially the same as for symmetric monoidal categories, where now the units and counits of the compact closed structure are internalized as follows:
...
\end{definition}
 ...
 In practice, we won't explicitly regard a  monoidal theory as a triple; rather, we will present multicoloured pros by first drawing the string diagrams for all of the generators and then equations string diagrams for the equations. 
...
This is a presentation for the pro, $\FinMonot$, of finite ordinals and monotone maps.
\end{example}
...
We see that $\FinOrd^\op$ embeds in this prop as well as in the prop $\Mat_S$ for some semiring $S$.  The pushout of this diagram is presented by adding this generator and equations to the presentation of $\Mat_S$.  This is a presentation for the prop of affine matrices over $S$.
\end{example}
...
Finite relations $\FRel=\Rel(\FSets)$, linear relations $\LinRel=\Rel(\FVect_k)$ are defined in the same way.
\end{example}
...
Span is not a Cartesian bicategory of relations because it is poset enriched.  As alluded to before, one could have given a similar axiomatization for non-poset enriched categories, as done in \cite{carbonia}, however this is much more difficult to work with because this notion requires coherence conditions.
...
It is not the case that monad maps correspond to functors between internal categories.  Functors are instead tight maps in the double category of spans o-nite sets and mono-tonf sets, which is outside of the scope of this thesis. 
...
\begin{lemma}
Given a two symmetric monoidal theories, and a distributive law between their corresponding multicoloured props, the induced pro has a monoidal theory
...
{\sf sfa} is a presentation for $(\Csp(\FinMonot),+)$.
\end{lemma}

The idea is that the pullback which takes spans to cospans gives you the distributive law.  Because $\FinOrd$ is presented in terms of 2 generators, one just has to check all the possible ways to push the generators of $\m$ past $\m^\op$.
...
\begin{lemma}
There is an equivalence of props $\cb\cong(\Span(\FSets),+)\cong (\Mat(\N),+)$.
\end{lemma}
...
In \cite{rosebrugh} they demonstrate that distributive laws in $\Span(\Set)$ correspond to strict factorization systems on the underlying internal category:  morphisms in the composite category $T;S$ of a distributive law $\lambda:S;T\to T;S$ can always be uniquely factored uniquely into the maps of $T$ followed by $S$.  
...
The normal form for special Frobenius algebras, and special commutative frobenius algebras both witness this fact. 
...
Consider the free pro generated by a Frobenius algebra. Then the circuits generated by the components of which are simply connected have a unique normal form. We use the spider notation on the right to refer to these simply connected components:
...
Because the simply connected circuits are reducible to each other, spiders connected by a single wire fuse:
...
In the case when the Frobenius algebra is also special, then simple-connectedness is equivalent to connectedness so that the fusion rule can be generalized to spiders connected by multiple wires:
...
The analagous normal forms hold for the free prop generated by commutative and special commutative Frobenius algebras, where wires are allowed to pass over each other.
...
String diagrams have been used in quantum theory for quite some time.  For example in the work of Penrose \cite{penrosei}.  In such settings, string diagrams have been used as (often heuristic) tools used for calculation. The more recent programme of ``categorical quantum mechanics,'' following the seminal paper of Abramsky and Coecke \cite{abramsky} has endeavoured to formulate finite dimensional quantum quantum mechanics using category theory.  In this setting,  string diagrams are formal mathematical objects: allowing certain essential features to be abstracted away from the category of finite dimensional Hilbert spaces.


To motivate the graphical this graphical treatment of finite dimensional quantum theory, we first establish some basic algebraic notations.
...
As a matter of notation: given some fixed dimension $d$, and a distinguished standard basis, we will call the  connected components of the special commutative $\dag$-Frobenius algebra corresponding the standard basis qudit {\bf Z-spiders}.  We will draw then in white, as follows:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=none] (0) at (1.5, 1.75) {};
...
The compact closed structure of $\CPM(\X)$ is inhereted from the doubling of the compact closed structure of $\X$ and compatible with the dagger such that  $\CPM(\X)$ is \dag-compact closed.
\end{definition}
...
This different perspective will prove useful for the purposes of calculation.  Therefore, one could have equivalently defined a this construction with respect to an identity on objects conjugation functor, rather than a $\dag$-functor.  Indeed further on in this thesis, this will be the most natural way to apply this construction.
...
The map $p_B$ is self-adjoint because:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (0) at (0, 3) {};
		\node [style=Z] (1) at (0, 3.75) {};
		\node [style=none] (2) at (-0.5, 2.25) {};
		\node [style=none] (3) at (0.75, 2.25) {};
		\node [style=none] (4) at (0.25, 2.25) {};
		\node [style=none] (5) at (0.75, 3.75) {};
		\node [style=none] (6) at (-0.5, 2) {};
		\node [style=none] (7) at (-0.5, 4.5) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw [in=-135, out=90] (2.center) to (0);
		\draw [in=90, out=-30, looseness=0.75] (0) to (3.center);
		\draw [in=270, out=270, looseness=1.25] (3.center) to (4.center);
		\draw [in=90, out=60, looseness=1.50] (1) to (5.center);
		\draw [in=90, out=-90, looseness=0.75] (5.center) to (4.center);
		\draw (2.center) to (6.center);
		\draw (1) to (0);
		\draw [in=-90, out=135] (1) to (7.center);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (16) at (2, 3.75) {};
		\node [style=Z] (17) at (2, 3.75) {};
		\node [style=none] (18) at (1.5, 3) {};
		\node [style=none] (19) at (2.75, 3) {};
		\node [style=none] (20) at (2.25, 3) {};
		\node [style=none] (21) at (2.75, 3.75) {};
		\node [style=none] (22) at (1.5, 2.75) {};
		\node [style=none] (23) at (1.5, 4.5) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw [in=-135, out=90] (18.center) to (16);
		\draw [in=90, out=-30, looseness=0.75] (16) to (19.center);
		\draw [in=270, out=270, looseness=1.25] (19.center) to (20.center);
		\draw [in=90, out=60, looseness=1.50] (17) to (21.center);
		\draw [in=90, out=-90, looseness=0.75] (21.center) to (20.center);
		\draw (18.center) to (22.center);
		\draw [in=-90, out=135] (17) to (23.center);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (8) at (4, 4.5) {};
		\node [style=Z] (9) at (4, 3.25) {};
		\node [style=none] (10) at (3.5, 3) {};
		\node [style=none] (12) at (4.5, 3.75) {};
		\node [style=none] (13) at (4.25, 4) {};
		\node [style=none] (15) at (3.5, 4.75) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw [in=-135, out=90] (10.center) to (8);
		\draw [in=180, out=45, looseness=1.25] (9) to (13.center);
		\draw [in=-180, out=0, looseness=1.25] (13.center) to (12.center);
		\draw [in=-105, out=105] (9) to (8);
		\draw [in=-90, out=135] (9) to (15.center);
		\draw [in=0, out=-30, looseness=0.75] (8) to (12.center);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (24) at (6.75, 4.5) {};
		\node [style=Z] (25) at (6.75, 3.5) {};
		\node [style=none] (26) at (6.25, 3.25) {};
		\node [style=none] (30) at (6.25, 4.75) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw [in=-135, out=90] (26.center) to (24);
		\draw [in=270, out=90] (25) to (24);
		\draw [in=-90, out=135] (25) to (30.center);
		\draw [in=-30, out=30] (25) to (24);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (31) at (7.75, 4) {};
		\node [style=Z] (32) at (7.75, 4) {};
		\node [style=none] (33) at (7.75, 3.25) {};
		\node [style=none] (34) at (7.75, 4.75) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw (33.center) to (31);
		\draw (32) to (34.center);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=none] (37) at (8.75, 3.25) {};
		\node [style=none] (38) at (8.75, 4.75) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
...
and it is idempotent because:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (0) at (11, 2.75) {};
		\node [style=Z] (1) at (11, 4.5) {};
		\node [style=Z] (2) at (10.5, 3.25) {};
		\node [style=Z] (3) at (10.5, 4) {};
		\node [style=none] (4) at (11, 5) {};
		\node [style=none] (5) at (11, 2.25) {};
		\node [style=none] (6) at (10, 2.25) {};
		\node [style=none] (7) at (10, 5) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		...
		\draw (4.center) to (1);
		\draw (1) to (3);
		\draw [in=90, out=-150, looseness=0.75] (3) to (6.center);
		\draw [in=-90, out=150, looseness=0.75] (2) to (7.center);
		\draw (2) to (0);
		\draw (5.center) to (0);
	\end{pgfonlayer}
\end{tikzpicture}
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (8) at (12.75, 2.5) {};
		\node [style=Z] (9) at (12.75, 4.75) {};
		\node [style=none] (10) at (12.75, 5.25) {};
		\node [style=none] (11) at (12.75, 2) {};
		\node [style=none] (12) at (11.75, 2) {};
		\node [style=none] (13) at (11.75, 5.25) {};
		\node [style=Z] (14) at (13.25, 4) {};
		\node [style=Z] (15) at (13.25, 3.25) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw (10.center) to (9);
		\draw (11.center) to (8);
		\draw [in=90, out=-150] (9) to (12.center);
		\draw [in=-90, out=150] (8) to (13.center);
		\draw [bend right, looseness=1.25] (14) to (15);
		\draw [bend right, looseness=1.25] (15) to (14);
		\draw [in=90, out=-30] (9) to (14);
		\draw [in=-90, out=45] (8) to (15);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (16) at (15.25, 2.75) {};
		\node [style=Z] (17) at (15.25, 4.5) {};
		\node [style=none] (18) at (15.25, 5) {};
		\node [style=none] (19) at (15.25, 2.25) {};
		\node [style=none] (20) at (14.25, 2.25) {};
		\node [style=none] (21) at (14.25, 5) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
...
To promote these classical mixtures to their own systems,  we regard the projectors onto quantum observables themselves as objects. First, we recall the Kar
...
\begin{description}
\item[\ \ Objects:] Pairs $(X,e)$ where $X$ is an object of $\X$ and $e:X\to X$ is a projector in $\mathcal I$.
...
Note that there is a  $\dag$-compact closed embedding $\X\ \rightarrowtail {\sf Split}_{{\mathcal I }\cup \{ 1_X | X \in \X\}}^{\dag}(\X)$. 
...
\begin{lemma}
Given two strongly complementary bases $B$ and $C$, performing two consecutive non-destructive measurements in each basis preserves no information:
\begin{align*}
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (39) at (11, 2.75) {};
		\node [style=Z] (40) at (11, 4.5) {};
		\node [style=none] (43) at (11, 5) {};
		\node [style=none] (44) at (11, 2.25) {};
		\node [style=none] (45) at (10, 2.25) {};
		\node [style=none] (46) at (10, 5) {};
		\node [style=X] (47) at (10.5, 4) {};
		\node [style=X] (48) at (10.5, 3.25) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw [bend left] (40) to (39);
		\draw (43.center) to (40);
		\draw (44.center) to (39);
		\draw [in=-90, out=135] (48) to (46.center);
		\draw [in=90, out=-135] (47) to (45.center);
		\draw (39) to (48);
		\draw (47) to (40);
		\draw [bend left] (47) to (48);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (49) at (13, 2.75) {};
		\node [style=Z] (50) at (13, 4.25) {};
		\node [style=none] (51) at (13, 4.75) {};
		\node [style=none] (52) at (13, 2.25) {};
		\node [style=none] (53) at (12, 2.25) {};
		\node [style=none] (54) at (12, 4.75) {};
		\node [style=X] (55) at (12.5, 3.5) {};
		\node [style=X] (56) at (12.5, 3.5) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw [bend left] (50) to (49);
		\draw (51.center) to (50);
		\draw (52.center) to (49);
		\draw [in=-90, out=135] (56) to (54.center);
		\draw [in=90, out=-135] (55) to (53.center);
		\draw (49) to (56);
		\draw (55) to (50);
	\end{pgfonlayer}
\end{tikzpicture}
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (57) at (15.25, 2.75) {};
		\node [style=Z] (58) at (15, 4) {};
		\node [style=none] (59) at (15, 4.5) {};
		\node [style=none] (60) at (15.25, 2.25) {};
		\node [style=none] (61) at (14, 2.25) {};
		\node [style=none] (62) at (13.75, 4.5) {};
		\node [style=X] (63) at (14, 3) {};
		\node [style=X] (64) at (14.5, 3.25) {};
		\node [style=Z] (65) at (14.75, 2.5) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw [bend left] (58) to (57);
		\draw (59.center) to (58);
		\draw (60.center) to (57);
		\draw (63) to (61.center);
		\draw [in=180, out=75] (63) to (58);
		\draw [in=135, out=-45] (64) to (65);
		\draw (65) to (57);
		\draw (64) to (63);
		\draw [in=-90, out=105] (63) to (62.center);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (67) at (16.75, 4.25) {};
		\node [style=none] (68) at (16.75, 5.25) {};
		\node [style=none] (69) at (17.75, 2) {};
		\node [style=none] (70) at (16.75, 2) {};
		\node [style=none] (71) at (16, 5.25) {};
		\node [style=X] (72) at (16.75, 3) {};
		\node [style=X] (73) at (16.75, 3.75) {};
		\node [style=Z] (74) at (17.25, 3.25) {};
		\node [style=Z] (75) at (16.75, 4.75) {};
		\node [style=X] (76) at (16.75, 2.5) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw (68.center) to (67);
		\draw (72) to (70.center);
		\draw [in=-150, out=150] (72) to (67);
		\draw [in=135, out=-45] (73) to (74);
		\draw (73) to (72);
		\draw [in=90, out=-30] (67) to (74);
		\draw [in=90, out=-30] (75) to (69.center);
		\draw [in=-90, out=165, looseness=0.75] (76) to (71.center);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=none] (78) at (19.5, 5.25) {};
		\node [style=none] (79) at (19.5, 2) {};
		\node [style=none] (80) at (18.75, 2) {};
		\node [style=none] (81) at (18.75, 5.25) {};
		\node [style=Z] (85) at (19.5, 4.75) {};
		\node [style=X] (86) at (18.75, 2.5) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw (85) to (79.center);
		\draw (86) to (81.center);
		\draw (80.center) to (86);
		\draw (78.center) to (85);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=none] (87) at (21.25, 5.25) {};
		\node [style=none] (88) at (21.25, 2) {};
		\node [style=none] (89) at (20.5, 2) {};
		\node [style=none] (90) at (20.5, 5.25) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
...
\begin{tikzpicture}[yscale=-1]
	\begin{pgfonlayer}{nodelayer}
		\node [style=Z] (91) at (23.25, 2.25) {};
		\node [style=none] (92) at (22.75, 1.5) {};
		\node [style=none] (93) at (23.25, 3) {};
		\node [style=ground] (95) at (23.75, 1.5) {};
		\node [style=X] (96) at (22.75, 1.5) {};
		\node [style=none] (97) at (22.25, 0.75) {};
		\node [style=none] (99) at (22.25, 0) {};
		\node [style=ground] (100) at (23.25, 0.75) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw (93.center) to (91);
		\draw [in=90, out=-150] (91) to (92.center);
		\draw [in=90, out=-30] (91) to (95);
		\draw [in=90, out=-150, looseness=0.75] (96) to (97.center);
		\draw (97.center) to (99.center);
		\draw [in=90, out=-30] (96) to (100);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=none] (93) at (23.25, 3) {};
		\node [style=ground] (95) at (23.25, 2.5) {};
		\node [style=none] (99) at (23.25, 0) {};
		\node [style=ground,scale=-1] (100) at (23.25, .5) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw (99.center) to (100);
		\draw (95) to (93.center);
	\end{pgfonlayer}
\end{tikzpicture}
$$

Therefore, any classical information prepared in a basis can not be recovered by measuring in a strongly complementary basis.
\end{lemma}
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		...
		\node [style=map] (1) at (0.45, 0.75) {$X^{b_1}$};
		\node [style=map] (2) at (1.55, 0.75) {$X^{b_m}$};
		\node [style=map] (3) at (1.55, -0.75) {$X^{a_n}$};
		\node [style=map] (4) at (0.45, -0.75) {$X^{a_1}$};
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		...
		\node [style=map] (14) at (42.2, -0.25) {$X^{a_1}$};
		\node [style=map] (15) at (43.25, -0.25) {$X^{a_2}$};
		\node [style=map] (16) at (44.3, -0.25) {$X^{a_3}$};
		\node [style=map] (17) at (42.2, 1.5) {$X^{b_1}$};
		\node [style=map] (18) at (43.25, 1.5) {$X^{b_2}$};
		\node [style=map] (19) at (44.3, 1.5) {$X^{b_3}$};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw [in=-135, out=90] (2.center) to (0);
		\draw [in=90, out=-45] (0) to (3.center);
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		...
		\node [style=map] (14) at (42.2, -0.75) {$X^{a_1}$};
		\node [style=map] (15) at (43.25, -0.75) {$X^{a_2}$};
		\node [style=map] (16) at (44.3, -0.75) {$X^{a_3}$};
		\node [style=map] (17) at (42.2, 1.75) {$X^{b_1}$};
		\node [style=map] (18) at (43.25, 1.75) {$X^{b_2}$};
		\node [style=map] (19) at (44.3, 1.75) {$X^{b_3}$};
...
So far, the two fragments of the ZX-calculus we have described are relatively simple; however, ZX-calculus gives a natural syntax for stabilizer circuits.  We review the stabilizer formalism to this end:
...
The qubit and quopit fragments of the ZX-calculus pick out precisely the qubit and quopit stabilizer circuits.  The qubit \cite{backensstab,backens2015} and qutrit \cite{qutrit} stabilizer ZX-calculus have been shown to be complete.  Howver, during the process of writing this thesis, an axiomatization was provided for the quopit ZX-calculus \cite{booth}, followed shortly by an even simpler presentation \cite{poor}:
...
It is also worth mentioning that, at least in the qubit case, there are complete presentations for the ZX-calculus where all phases are allowed \cite{zxcompletea,zxcompletea}.  This gives a graphical presentation for the full subcategory of $\Mat_\C$ whose objects are powers of $2$; which is of course not classically simulatable in polynomial time.
...
To accommodate for this, so called ``H-boxes'' were devised in the qubit case by \cite{zh}:
...
Therefore, one should not confuse an $H$-box with one input and one output (drawn in grey) with the antipode for the Hopf algebra (drawn in black) for the Z and X spiders.
