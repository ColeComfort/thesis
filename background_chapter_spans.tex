...
Combining these two involutions we have:
...
Recall the of affine matrices. One might seek to find a similar presentation for relations internal to affine matrices.
...
A map $f$ in a restriction category is called a {\bf partial isomorphism}, in case there exists a {\bf parital inverse} $g$ of $f$ so that $f;g=\bar f$ and $g;f = \bar g$. Denote the category of partial isomorphisms of $\X$ by $\ParIso(\X)$.
...
Given a discrete Cartesian restriction category $\X$, $\ParIso(\X)$ is a discrete inverse category.
\end{lemma}

To simplify notation, given a finitely complete category $\X$ define $\ParIso(\X)=\ParIso(\Par(\X))$.  That is, the category whose maps are spans of monomorphisms in $\X$.

At least as far as the literature is concerned, the other direction is not so trivial (we give a simpler characterization in terms of adding counits to the inverse products in Proposition \ref{cor:copy}).  We will first introduce the more CoPara construction, which freely adds an effect to every object in a way that is compatible with the tensor product.
