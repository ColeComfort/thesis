...
\end{description}
\end{definition}
The graphical calculus for pointed profunctors is essentially the same as for pointed categories (recall that the graphical calculus for pointed categories which we used is already adapted to that of pointed categories).  Except now, due to the two different Yoneda embeddings, we can factorize the domain and codomain of maps.  For example, consider the action unit and counit for the tensor product:
$$
  \begin{tikzpicture}[scale=1.5]
    \node[Pants, top] (pants) {};
...
\xRightarrow{\eta^I}
  ...
  \end{tikzpicture}
$$
These diagrams have a striking resemblance to proof nets, corresponding to the inverses of the monoidal strengths.  
...
The composite is defined as follows where $\tilde{(f^\sharp, g^\sharp)}$ is the canonical element induced by the coend on $f^\sharp$ and $g^\sharp$:
