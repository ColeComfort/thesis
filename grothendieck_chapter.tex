...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		...
		\node [style=dot] (47) at (11.5, 7) {};
		\node [style=dot] (48) at (11.5, 6.25) {};
		\node [style=none] (49) at (9.5, 9.5) {};
		\node [style=none] (50) at (9, 9.5) {};
		\node [style=none] (51) at (10.25, 9.5) {};
		\node [style=dot] (52) at (12.25, 8) {};
		\node [style=dot] (53) at (12.25, 7.25) {};
		\node [style=dot] (54) at (13.75, 9.25) {};
		\node [style=dot] (55) at (13.75, 8.5) {};
...
These diagrams are very closed to proof nets.  We can obtain the following profunctor
$$
(\X(-,=), \alpha_{X,Y,Z}):
...
The equality of these maps is modulo the equivalence relation generated by two congruences:

\begin{itemize}
\item
All scalars are equal to $(1_\one,1_*)$.


\item 
For two scalar-free parallel maps

$$ (P,f),(Q,g): (\X^n,X) \proarrow (\X^n,X)$$

are equivalent when $Q$ and $P$ are normalized to the same stratified spider $\nu_0:P\Rightarrow S \Leftarrow Q:\nu_1$ where moreover, the normalization agrees on points so that $\nu_0(f)=\nu_1(g)$.
...
This is moreover a strict monoidal category.  The tensor product is given by the tensor product in $\Prof^*$, so that the projection functor $\pi_0:\Pi F_\X\to \sfa$ is strict monoidal.
...
Moreover, $\eta^\otimes$ is now a proper equality:
$$
\begin{tikzpicture}[scale=1.5]
    \node[Pants, top,bot] (A) at (0,0) {};
    \node[Copants,bot, anchor=leftleg] (B) at (A.leftleg) {};
...
To this end, we want to force all components to be connected. 

First remark that for any  finite list of objects $X=[X_0,\ldots, X_{n-1}]$ in $\X$ there is an idempotent $(s_X, s_X^\sharp)=e_X:X\to X$ in   $\Pi F_{\X}$ where $s_X$ is the fully connected spider from $\X^n\to\X^n$ and $s_X^\sharp$ is the tensor factorized identity $s_X^\sharp = \otimes_{i=0}^{n-1}1 _{X_i}$ regarded as an element of the spider.
For example 
...
This determines a strict monoidal category in which all shapes connected to the boundary are connected.
...
\item[Maps:] 
$$\dfrac{(P,f);X\to Y \hspace*{.5cm} \in \Pi F_\X\ \text{ where the boundaries of $P$ are fully connected} }{(P,f);X\to Y \hspace*{.5cm} \in N\X}$$
...
This monoidal category is very close to the strictification of a monoidal category; however, it is not quite there.  There is no way to eliminate the scalars  (maps of the form $[]\to[]$) in this category. For example, the following equation does not hold:
