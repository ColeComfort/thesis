...
In this Chapter, we attempt to understand proof nets/the categorical semantics for the scalable ZX-calculus by  categorifying Frobenius algebras and the spider theorem.

Throughout this thesis, we have been drawing string diagrams in monoidal, or symmetric monoidal categories in which our familiar notions of monoids and Frobenius algebras and so on live.  However, if we move to the higher dimensional setting of monoidal 2-categories, all of the equalities defining the axioms for these structures become natural transformations, satisfying more subtle coherence laws.   For this section, I will assume that the reader has some familiarity with monoidal 2-categories. Recall the categorified notion of a monoidal in a monoidal 2-category:
...
Consider $\Cat$ as a symmetric monoidal 2-category where the tensor product is given by the product.
...
One should think of the convex regions of space inside of the tube as the category itself; wherein one can apply rewrite rules coming from the equational theory of the category.  A functor is drawn as a membrane; functoriality means that things can pass up through the membrane:
...
$$
 ...
 If $\X$ is a monoidal category, then for every map with a binary tensor factorization of the codomain  $f:X\otimes Y\to Z$ there is a pointed functor:
...
So we see that, being remeniscent of proof nets, this gives us a way to decompose the inputs of string diagrams into tensor factors.  
...
This is unsatisfying, because of the one-sided nature of the way we are able to factorize the tensor product of maps.  Instead of working in a Cartesian category, we want to work in a cartesian bicategory!
...
$$
\dfrac{\mathcal{C}\proarrow \mathcal{D}\quad \in\ \Prof}
{\mathcal{C}^\op\times\mathcal{D}\to\Sets \quad \in\ \Cat}
$$

The composition of profunctors $P:\mathcal{C}\proarrow \mathcal{D}$ and $Q:\mathcal{D}\proarrow \mathcal{E}$ is given by the coend:
$$
P;Q(C,E) := \int^{D \in \mathcal{D} } P(C,D) \times Q(D,E) 
$$

The identity profunctor on $\mathcal C$ is given by the hom functor ${\mathcal C}: \mathcal{C}^\op\times\mathcal{C}\to\Sets$.

Formally, the coend of a functor $F:\mathcal{C}^\op\times\mathcal{C}\to \Sets$ is given by the coequalizer diagram (in $\Cat$):

$$
\coprod_{C_1,C_2 \in \mathcal{C}} {\mathcal{C}}(C_1,C_2) \times F(C_1,C_2) \rightrightarrows \prod_{C \in \mathcal{C}} F(C,C) \to \int^{C \in \mathcal{C}} F(C,C)
$$
...
A profunctor $\mathcal{C}\proarrow \mathcal{D}$ is {\bf representable} when it is naturally isomorphic to the profunctor $F_*:={\mathcal D}(F-,=)$ for a functor $F: \mathcal{C}\to \mathcal{D}$.


Dually, a profunctor $\mathcal{D}\proarrow \mathcal{C}$ is {\bf corepresentable} when it is naturally isomorphic to the profunctor $F^*:={\mathcal D}(-,F=)$ for a functor $F: \mathcal{C}\to \mathcal{D}$.
...
talk about coherences

\begin{theorem}
Given a monoidal category $\X$, and a connected diagram in $\Prof$ generated by the $1$-cells of the pseudo-Frobenius structure induced by $\X$ one can always reduce the diagram to spider normal form by repeated application of $\phi^L,\Psi^R,\eta^\otimes$ and $\eta^I$. Moreover, this is strictly confluent in the sense, so that any $2$-cells witnessing the reduction to the spider normal form are equal.


Furthermore, given any {\em not-necessarily connected} diagram can be reduced to a normal form given by stratifying the connected components into spiders.
\end{theorem}


We can do the same thing we did with pointed categories, and look inside the profunctors:
...
$$
 (F:\X\proarrow\Y, f\in F(X,Y)),  (G:\X\proarrow\Y, g\in (X,Y)):(\X, X\in\X_0)\to (\Y, Y\in\Y_0)
$$
...
The graphical calculus for pointed profunctors is essentially the same as for pointed categories.  Except now we can factorize the domain and codomain of maps.  For example, consider the action unit and counit for the tensor product:
...
These diagrams are very closed to proof nets; for example we can internalize the associators and unitors:

DRAW The associator and unitor diagrams

However, we are still living in a monoidal 2-category, moreover, the tensor/cotensor and unit introduction and removal are not inverse to each other; rather, they are adjoint. What is going on here?  To get a deeper perspective, we have to consider the following structure:
...
\begin{itemize}
\item $(F_\X)(n) = \X^n$
\item $(F_\X){n,m}$ takes diagrams in $\esfa$ to (stratified) spiders in $\Prof$ composed of the monoidal structure of $\X$.
...
and
$$(g;g^\sharp):Y=[Y_0,\ldots, Y_{k-1}]\to Z= [Z_{0},\ldots, Z_{\ell-1}]$$
...
Talk about unitors and associators being honest maps on connected components
