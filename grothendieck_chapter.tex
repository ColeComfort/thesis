...
In  \cite{wilson} they reprove the coherence theorem for monoidal categories; constructing a strict monoidal category equivalent to a given monoidal category by inductively building up the strictification functor.
 Then they regard the coherence maps of the (strong) monoidal functor as generators which we called  tensor and unit introduction.  The inverses to the coherence maps regarded as generators which we called cotensor and unit removal.
...
Lax and oplax monoidal functor boxes seem to have first be described in \cite{Cockett1999} where they were used in order to give a graphical calculus for  linearly distributive functors; they are also discussed in \cite{Cockett2018}. Functor boxes were independently rediscovered by \cite{mellies}, where they were also applied to strong monoidal functors. The clearest exposition of lax/oplax/frobenius/strong monoidal functors in my opinion is contained in \cite{Micah}.  Take a  functor $F:\X\to \Y$.  Maps in $\X$ drawn within a box within $\Y$  where the domain comes through the bottom boundary and the codomain leaves through the top boundary.  Therefore, every map $f:X\to Y$ in $\X$ is simply regarded as a map $F(f):F(X)\to F(Y)$ in $\Y$.  When $F$ is moreover lax monoidal, then maps in $\X$ are drawn within the Poincar\'e dual of shapes generated by monoid  drawn within $\Y$.  That is to say, given any box, finitely many wires can enter the box but only one can leave.  When the functor is moreover strong monoidal; then we drawn maps within the Poincar\'e dual of shapes generated by a monoid with an inverse comonoid.
%then the components $\mu_{X,Y}:F(X)\otimes^\Y (Y) \to F(X \otimes^\X Y)$ are drawn so that multiple inputs merge into the box and $\eta:I^\X\toI^\Y$ is drawn as introducing a box from empty space.  That is to say, the maps $f:X\otimes^\X Y\to Z$ in $\X$ are drawn within the Poincar\'e dual of a the multiplication for a monoid denoting  $F(f):F(X)\otimes^\Y F(Y)\to F(Z)$; and maps $f:I^\X\to X$ are drawn inside the .  When $F$ is strong monoidal, then then one can moreover draw string diagrams within the Poincar\'e dual of a comonoid which is inverse to the monoid.
Therefore, take the any strictification of a monoidal category $S:\X\to \bar{\X}$.  Drawing string diagrams in $\bar \X$ with functor boxes for $S$ reproduces the  generators and relations approach to strictification of \cite{wilson}.


%As far as the graphical calculus for linearly distributive functors, is concerened, 


Let me give some more history about functor boxes.
The graphical calculus for linearly distributive functors is more nuanced than that for monoidal functors because proof nets for linearly distributive categories are more nuanced than string diagrams monoidal categories.  In particular, one can draw diagrams that appear to be proof nets which are not valid.  One way to tell if a proof net is valid is via the sequentialization procedure described in  \cite[\S 2.4]{Blute1996}.  This is in some sense a linearly distributive analogue to the inductive strictification procedure for monoidal categories described in  \cite{wilson}; although it is not itself a strictification procedure.  This  involves inductively applying ``boxes'' to a proof net.  If the whole circuit is boxed when the procedure terminates, then one can conclude that the proof net is well-formed. This procedure actually precedes the notion of functor box and was reframed in terms of linearly distributive functor boxes in \cite{Cockett1999}. However the linearly distributive setting, there is a complication with units: extra virtual wires called ``thinning links'' are needed to unambiguously decide the connectivity of the units.  Without thinning links then a proof net can be spuriously identified with distinct maps.
...
Inspired by the coherence via universality approach of Hermida \cite{Hermida2001,hermida}, which I will touch on later, I wondered if there is a universal way to produce the strictification of a monoidal category in a way that simultaneously reproduces proof nets for monoidal categories.  The appeal of a universal approach is that it would provide  a general recipe to create string diagrams for other algebraic structures in monoidal categories.  In the end, I didn't manage to succeed; however, I think that the route that I took in order to attempt this is insightful, and could potentially be useful to others. 
