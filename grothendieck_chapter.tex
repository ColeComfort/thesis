...
%
%\begin{definition}
%The symmetric monoidal 2-category of {\bf pointed Categories} is the coslice category $\Cat_*:=1/\Cat$.  Explicitly, this has:
%
%\begin{description}
...
%
%\begin{definition}
%The monoidal 2-category of pointed profunctors, $\Prof_*$, has
%
%
%\begin{description}
%\item[0-cells:]
...
\begin{theorem}
Given a monoidal category $\X$, the monoidal strictification of $\X $ has:
...
We  hope that the highly conceptual level of the construction opens up avenues for finding string diagrams for algebraic structures; our goal being to find canonical ways to glue monoidal categories together and obtain new kinds of string diagrams thereof.  This kind of problem has previously been considered in the setting where one wants to glue together monoidal categories along multiple different monoidal functors \cite{lobski}.  The motivation therein is to give a semantics for ways to explain systems which are stratified into multiple different layers, although they obtain a monoidla 2-category, rather than an ordinary monoidal category.
...
\begin{definition}
A {\bf pseudomonoid} in a monoidal 2-category is an object $\mathcal C$ equipped with two 1-cells $-\otimes =:\mathcal{C} \otimes \mathcal{C} \to \mathcal{C}$ and $I:1\to \mathcal C$ drawn as follows
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
\begin{definition}
The symmetric monoidal 2-category of {\bf pointed categories} is the coslice category $\Cat_*:=\one/\Cat$.  Explicitly, this has:
...
\end{tikzpicture}
$$
Think if the maps maps inside the category as ``living'' within the convex space contained with this cobordism; wherein one can apply rewrite rules coming from the equational theory of the category.  
...
And moreover, the projection map $\pi_0:F_\X\to \one$ is a fibration of multicategories.  He shoes that there is an equivalence of 2-categories between the multicategorical functor category $[\one,\Cat]$ and the subcategory of the slice category of multifibrations over $\one$.  
In some sense, this equivalence can be regarded as coherence for monoidal categories.
...
\begin{tikzpicture}
	...
	\begin{pgfonlayer}{edgelayer}
		...
		\draw [colour=blue, dashed] (102.center) to (103.center);
		\draw [colour=blue, dashed] (103.center) to (104.center);
		\draw [colour=blue, dashed] (104.center) to (105.center);
		\draw [colour=blue, dashed] (105.center) to (102.center);
		...
		\draw [colour=blue, dashed] (116.center) to (117.center);
		\draw [colour=blue, dashed] (117.center) to (118.center);
		\draw [colour=blue, dashed] (118.center) to (119.center);
		\draw [colour=blue, dashed] (119.center) to (116.center);
		...
		\draw [colour=blue, dashed] (135.center) to (136.center);
		\draw [colour=blue, dashed] (136.center) to (137.center);
		\draw [colour=blue, dashed] (137.center) to (138.center);
		\draw [colour=blue, dashed] (138.center) to (135.center);
...
\begin{tikzpicture}
	...
	\begin{pgfonlayer}{edgelayer}
		...
		\draw (2.center) to (1);
		\draw [colour=blue, dashed] (6.center) to (7.center);
		\draw [colour=blue, dashed] (7.center) to (8.center);
		\draw [colour=blue, dashed] (8.center) to (9.center);
		\draw [colour=blue, dashed] (9.center) to (6.center);
		...
		\draw [colour=blue, dashed] (20.center) to (21.center);
		\draw [colour=blue, dashed] (21.center) to (22.center);
		\draw [colour=blue, dashed] (22.center) to (23.center);
		\draw [colour=blue, dashed] (23.center) to (20.center);
		...
		\draw [colour=blue, dashed] (34.center) to (35.center);
		\draw [colour=blue, dashed] (35.center) to (36.center);
		\draw [colour=blue, dashed] (36.center) to (37.center);
		\draw [colour=blue, dashed] (37.center) to (34.center);
...
Furthermore, given any {\em not-necessarily connected} diagram can be reduced to stratified spider normal form using the same collection of $2$-cells.
\end{lemma}
...
These diagrams are very closed to proof nets.  We can obtain the associator regarded as an element of the $\hom$-profunctor:
$$
(\X(-,=), \alpha_{X,Y,Z}):
...
In the setting of proof nets, the tensor is inverse to the cotensor and the unit introduction is inverse to the unit removal.  Howver, slightly closer than in $\Cat_*$, in $\Prof_*$ these 2-cells are at least adjoint.  We want to turn the adjunctions into equalities somehow.  We attempt this by regarding the normalization of the diagrams in $\Prof_*$ as the maps themselves.  
...
The idea of regarding monoidal categories as displayed categories is partially motivated by the work of \cite{blanco}, transporting the work of Hermida in the multicategorical setting to the polycategorical setting:  where the inputs and outputs are now both lists of objects.  In this work, they construct the Benabou-Grothendieck {\em *-polycatgory}, coming from a {\em displayed polycategory} $\one\to \Prof$, where $\Prof$ is regarded as a 2-polycategory.  In this *-polycategorical setting, their notion of a Frobenius algebra is precisely  a displayed categories from the terminal *-polycategory.
...
To proof nets, we would want all components to be connected.
...
Moreover, the top and bottom boundaries need not be connected {\bf to each other}, so  we still don't recover the other equation coming from the unit adjunction:
...
One could impose these two equations as quotients, apparently recapturing proof nets; however, taking quotients is generally undesirable, as it is ill-behaved.

%Connecting idempotents
%Subcategory of Karoubi envelope has another monoidal product

On top of this,  when described the normalization proceedure, we used the symmetric monoidal structure of $\Prof$ to move the scalars to the side in to be able to normalize spiders.  It is not the case that scalars commute with maps in general, so this wasn't going to work from the beginning.  
...
Nevertheless, I think there is a lot more to be done here; a proof of the uniqueness of the stratified spider normal form, or some variation of such,  being the most important. Despite  not giving a semantics for proof nets for monoidal categories, this still does give string diagrams which ought to work at least in the case when there are no nontrivial scalars.  
