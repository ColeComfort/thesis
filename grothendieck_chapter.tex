...
Recall the presentation of the strictification of a monoidal category in terms of generators and relations, whose string diagrams we have referred to as proof nets for monoidal categories:
\begin{definition}
...
\begin{definition}
The symmetric monoidal 2-category of {\bf pointed categories} is the coslice 2-category $\Cat_*:=\one/\Cat$.  Explicitly, this has:
...
\end{tikzpicture}
$$
Think if the maps maps inside the category as ``living'' within the cylinder; wherein one can apply rewrite rules coming from the equational theory of the category.  
...
Indeed, Hermida \cite{hermida}, showed that when one regards an (unbaised) monoidal category as a multifunctor $F:\one\to\Cat$, by computing the following pullback in the 2-category of 2-multicategories, the Grothendieck category $\int F_\X$ is precisely a representable multicategory:
