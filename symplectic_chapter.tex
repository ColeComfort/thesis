...
\begin{remark}
Although $\LinRel_k$ is a bicategory of relations, $\Lag\Rel$ is not.  No matter, which Frobenius algebra we chose, it is not laxly natural with respect to both the phase shifts and the fourier transform of the phase shifts:
...
\begin{definition}
Let $\Aff\Co\Isot\Rel_k^M= {\sf Split}_{\{p_Z^{\otimes n}, 1_n | n \in \N \}}(\Aff\Co\Isot\Rel_k)$ denote the two-coloured prop generated by splitting $p_Z$ in $\Aff\Co\Isot\Rel_k$.
