...
Notice that unlike the qudit Weyl group, there is no phase-factor.
...
On the other hand $\Aff\Lag\Rel_{\F_2}$ has already been studied in other terms. Spekkens first introduced his toy model as a noncontextual hidden variable analogue to qubit stabilizer quantum mechanics, satisfying the so-called ``knowledge-balance principle'' \cite{spekkens}.
...
\end{tikzpicture}
$$
Such that the phases are unitaries with respect to the dagger $(-)^\dag:=(\bar(-))^*$.
\end{definition}
...
\begin{tikzpicture}
	...
	\begin{pgfonlayer}{edgelayer}
		...
		\draw [style=classicalwire, in=-90, out=90, looseness=0.50] (584.center) to (593.center);
		\draw [style=classicalwire, in=-90, out=90] (583.center) to (594.center);
...
\begin{tikzpicture}
	...
	\begin{pgfonlayer}{edgelayer}
		...
		\draw [style=classicalwire, in=225, out=90, looseness=0.75] (308) to (312);
		\draw [in=225, out=120, looseness=0.50, style=classicalwire] (314) to (311);
...
\begin{tikzpicture}
	...
	\begin{pgfonlayer}{edgelayer}
		...
		\draw [style=classicalwire, bend left=15, looseness=0.50] (171) to (172);
		...
		\draw [style=classicalwire, bend left=15, looseness=0.50] (170) to (173);
	\end{pgfonlayer}
\end{tikzpicture}
=
\begin{tikzpicture}
...
$$
\end{remark}
The controlled voltage source is the electrical circuit analogue of a $X$-phase correction in quantum circuits.  Similarly, the controlled current source is the transpose of a nondestructive measurement in the $Z$-basis. 

It is strange that affine isotropic relations (of non-maximal dimension) show up in electrical circuits, but not in stabilizer circuits.
Just as Spekkens' toy model can be interpreted as an epistemically restricted toy theory of quantum circuits; by dualizing things, we could also ask what properties of quantum mechanics hold in $\Aff\Co\Isot\Rel_{\F_p}$.  This  is an empistemically restricted toy theory of quantum circuits; albeit the epistemic restriction is going in the wrong direction.  In other words, there is a {\em minimum} amount of knowledge the observer has about a quantum system.
