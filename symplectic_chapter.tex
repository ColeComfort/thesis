...
\begin{proof}
We know that the Clifford group is defined as the normalizer of the Weyl group so that a Clifford operator is defined by its action on Weyl operators. Given an $n$ quopit Clifford operator $C$  and $(z,x)\in \F_p^{2n}$, there exists  an isomorphism $C_L:\F_p^{2n}\to \F_p^{2n}$ and a vector  $C_a\in \F_p^{2n}$ such that:
...
Notice that unlike the qudit Weyl group, there is no phase-factor; as affine Lagragain relations only have scalars 0 and 1.
...
Because of the reasons we mentioned earlier $\Aff\Lag\Rel_{\F_2}$ is not isomorphic to $\Stab_2$.  
...
Later, this was formalized in terms of symplectic geometry, where he showed that the pure states in the toy model are affine Lagrangian subspaces over $\F_2$ and the reversible evolution is given by affine symplectomorphisms: the knowledge balance principle corresponding to the preservation of the symplectic form \cite{spekkens2016quasi}. 
 ...
 The general idea is that the one interprets the Lagrangian subspace as the epistemic state of the position and momentum variables (ie, what the observer knows about the state); moreover, the symplectic dual of the Lagrangian subspace is interpreted as the ontic state (the physical or real, properties of the state).  T Galois connection induced by the symplectic dual is interpreted as the knowledge balance principle.  If the observer never measures anything, so that the ontic state is equal to the empistemic state then:
...
We extend the notion of a symplectic stabilizer group to the mixed setting, using the standard notion of stabilizers for density matrices:
...
\end{tikzpicture}
$$
By Proposition \ref{prop:uniqueness}, this mapping is a full functor. For faithfulness, take maps $\hat f$ and $\hat g$ sent to the same affine coisotropic relation with purifications $(m,f:0\to n+ m)$ and  $(\ell,g:0\to n+ \ell )$ where without loss of generality $m\geq \ell$.
...
The maps are affine coisotropic subspaces, interpreted as the epistemic states and their symplectic duals as the ontic states. When part of the epistemic state is discarded, then then the dimension can increase; meaning that the dimension of the ontic state decreases. This will make more sense when we talk about measurement.




{\em Absolutely remarkably}, and seemingly out of nowhere, the quopit stabilizer codes with trivial affine phase (no Weyl gates) can be expressed, modulo invertible phase, in terms of an iterated $\CPM$ construction with respect to the orthogonal complement at the inner level, and the complex conjugation at the outer level:
\begin{corollary}
...
$$
\end{remark}
The controlled voltage source is the electrical circuit analogue of a $X$-phase correction in quantum circuits.  Similarly, the controlled current source is the transpose of a nondestructive measurement in the $X$-basis. However the voltmeters and ammeters have no quantum analogue.


Just as Spekkens' toy model can be interpreted as an epistemically restricted toy theory of quantum circuits; by dualizing things, we could also ask what properties of quantum mechanics hold in $\Aff\Isot\Rel_{\F_p}^M$.  This  is an ``ontologically restricted'' toy theory of quantum mechanics; or equivalently an epistemically ``co-restricted'' toy theory. In other words, there is a {\em minimum} amount of knowledge the observer has about the ontic state.

Because of the incompatibility of isotropic relations with quopit stabilizers; this gives evidence that the epistemic state of the observer acts on the ontic state, rather than the other way around.  In classical mechanics, witnessed by this interpretation of controlled current and voltage sources as well as voltmeters and ammeters, we see that the observer and the physical state live on the same footing.

\section{Error correction}
\label{sec:qec}


TODO SYMPLECTIC STINESPRING DILATION

TODO Weyl group discretization of errors Nielsen and Chuang
...
The relationship between logical and physical qubits can be understood in terms of pictures.  We will draw the doubled string diagrams for calculation and bastard spider diagrams because the presentation is cleaner.
...
\ \right\rrbracket
$$
Splitting this projector fixes a symplectic basis $\{b_1,\ldots, b_{n-k}\}$ for $L^\omega$, where the elements of the basis are pairwise orthogonal with respect to the symplectic form.  That is to say, they commute, which fixes the possible measurement outcomes:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
\ \right\rrbracket
$$
Suppose that Alice encodes a state and sends it to Bob on a noisy quantum channel with  error $W(e)$.  To detect the error apply the non-destructive measurement with respect to our chosen basis on the last $n-k$ wires in the $Z$-basis conjugated by $U$:
...
\begin{tikzpicture}
	...
	\begin{pgfonlayer}{edgelayer}
		...
		\draw [style=classicalwire, in=-120, out=90] (378.center) to (379);
		\draw [style=classicalwire, in=-15, out=90, looseness=0.75] (379) to (380);
		\draw [style=classicalwire, in=90, out=-60] (379) to (377.center);
