...
In this subsection, we show that, when $p$ is an odd prime, the prop of affine Lagrangian relations over $\F_p$  is isomorphic to quopit stabilizer circuits modulo invertible scalars.  To show this, we must first recall some very important results, relating stabilizer circuits to symplectic geometry.  First:
...
Note that for qubits the phase is not $\chi(C_a(z,x))$, so this is not true.
...
$$
{\mathcal W}(z,x) {\mathcal W}(z',x') = \chi(z_0x_1-x_0z_1) {\mathcal W}(z',x') {\mathcal W}(z,x) $$


Take $P=\mathcal{W}(z,x)$ and  $P'=\mathcal{W}(z',x')$.
There are two ways to compute  $(P'P)C(P'P)^\dag$;  we could compute the conjugation of $C$ by $P'P$; or we could first compute the conjugation of $C$ by $P$ and then conjugate by $P'$.  In either case, to obtain a Weyl operator in normal form (so that tensors of  powers of $\cal Z$ are followed by tensors of powers of $\cal X$), we must commute the Paulis past each other.  This means that:
...
$$
\chi(C_a){\cal W}(C_L(z,x)) D {\cal W}(C_L(z,x))^\dag
=
 \chi(C_a+D_a(C_L(z,x)))    {\mathcal W}( D_L(C_L(z,x)))
$$
...
Recall that, up to scalars, the $n$-quopit Clifford group is generated by the Pauli $\mathcal X$-gate, the controlled-$\mathcal X$ gate, the Fourier transform $\mathcal F$ and the phase gate $\mathcal S$ and scaling gates $\mathcal{M}_a$ for all $a \in \F_p^*$.
...
For every odd prime $p$ and $n \in \N$, there is a bijection between (nonempty) affine coisotropic subspaces of $\F_p^{2n}$ and $n$-quopit stabilizer states modulo nonzero scalars.
\end{lemma}
\begin{proof}
Recall that every stabilizer state is of the form $C|0\rangle^{\otimes n}$ for some Clifford $C$.  Therefore, the stabilizers are of the form $$\chi( C_L^{-1}(z_0,\cdots,z_{n-1},0,\cdots, 0)) {\cal W}( C_L^{-1}(z_0,\cdots,z_{n-1},0,\cdots, 0))$$ so that:


\begin{align*}
C|0\rangle^{\otimes n}
&=
\chi( C_L^{-1}(z_0,\cdots,z_{n-1},0,\cdots, 0)){\cal W}(C_L^{-1}(z_0,\cdots,z_{n-1},0,\cdots, 0)) \\
&\ \ \ \cdot C {\cal W}(C_L^{-1}(z_0,\cdots,z_{n-1},0,\cdots, 0))^\dag |0\rangle^{\otimes n}\\
&=
\chi( C_L^{-1}(z_0,\cdots,z_{n-1},0,\cdots, 0))C_L^{-1}(z_0,\cdots,z_{n-1},0,\cdots, 0)) C |0\rangle^{\otimes n}
\end{align*}

Because $C_L$ is an isomorphism, there are exactly $n$ such stabilizers; moreover, given any two stabilizers $\chi(a){\cal W}(z,x)$ and $\chi(a'){\cal W}(z',x')$:

\begin{align*}
\chi(a'){\cal W}(z',x') \chi(a){\cal W}(z,x)U|0\rangle^{\otimes n} &= \chi(a'){\cal W}(z',x')U|0\rangle^{\otimes n}\\
&= U|0\rangle^{\otimes n}\\
&= \chi(a){\cal W}(z,x)U|0\rangle^{\otimes n}\\
& =  \chi(a){\cal W}(z,x) \chi(a'){\cal W}(z',x')U|0\rangle^{\otimes n}   
\end{align*}

So that ${\cal W}(z',x'){\cal W}(z,x)={\cal W}(z,x) {\cal W}(z',x')$, and thus $\omega((z,x),(z',x'))=0$.


Conversely, given any affine Lagrangian subspace $L+a\subseteq \F_p^{2n}$; the stabilizer state is given by:
$$
\dfrac{1}{p^n}\sum_{v \in L}\chi(\omega(a,v)) {\cal W}(v)
$$
\end{proof}

Explicitly, the state $|0\rangle$ with an affine Lagrangian subspace:
...
\begin{proof}
Take a state $f:0\to n$ in $\Aff\Lag\Rel_k$.  Then $(z,x) \in f$ if and only if $W(z,x)$ is stabilized by $f$.  Since subspaces are equal if and only if they have the same elements.
\end{proof}
...
\end{cases}
$$
is a symmetric monoidal isomorphism, where $G$ is the bijection  between affine Lagrangian subspaces and stabilizer states modulo scalars.
\end{theorem}
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=map] (40) at (179.75, -4) {$\hat g$};
		\node [style=map] (44) at (181, -4) {$\hat f$};
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=map] (64) at (204.75, 0) {$G\left(\hat g\right)$};
		\node [style=none] (65) at (204, 1.25) {};
		\node [style=Z] (66) at (205.5, 1) {};
		\node [style=map] (67) at (206.25, 0) {$G\left(\hat f\right)$};
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=map] (295) at (233.75, 0) {$G\left(\hat {f;g}\right)$};
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=map] (259) at (222.25, 0) {$G\left(\hat g\right)$};
		\node [style=map] (260) at (223.75, 0) {$G\left(\hat f\right)$};
...
A basis for the affine Lagrangian subspace over $\F_p$ corresponds to the stabilizer tableau for a pure quopit stabilizer state (ie a stabilizer tableau on $n$ qudits with dimension $n$).
...
If we drop the affine shifts, we get a smallre fragment of stabilizer circuits:
...
Notice that  $\Aff\Lag\Rel_{\F_2}$ is not isomorphic to $\Stab_2$.  However, this does give the semantics for a known structure:
...
\begin{corollary}
$\Lag\Rel_{\F_2}$ is a presentation for the pure states in Spekkens' qubit toy model.
\end{corollary}
...
Every stabilizer code has an associated affine isotropic subspace and affine coisotropic subspace taking the symplectic complement of the linear component of the affine subspace. However, as noted in Remark  \ref{rem:xdisc}, $\Aff\Isot\Rel_{\F_p} \not \cong \Aff\Co\Isot\Rel_{\F_p}$, so their compositions as affine relations are different. The interpretation of the doubled zero postselection as the quantum discard map is not sound with respect to relational composition.
...
$$\ZXA \leftarrow \LinRel_{\F_2} \rightarrow \Aff\Co\Isot\Rel_{\F_p}^M$$

We can afford to do this because of the presentation of $\ZXA$ in the previous chapter. However, this is not very satisfying, because we only have a presentation for qubit nonlinear post-processing; and the symplectic formalism only gives a semantics for quantum circuits without phase-shift gates.   This gives more motivation to actually work out a presentation for the qudit version of $\ZXA$.
...
The nonlinear processing imposes equations which mean that the subset is no longer affine, for example:

TODO 

However, not every subset of $\F_p^{2n}$ can be constructed in this way.  In particular, there is no quantum circuit that can simultaneously copy the position and momentum.  To the knowledge of the author, this question has not yet been addressed, presumably, because classical processing and stabilizer codes are usually presented on different footings, as opposed to both being regarded themeselves as subspaces.

 


We have not given an undoubled completeness proof for $\Aff\Co\Isot\Rel_k$; instead this will be presented in a forthcoming paper.  The completeness proof mirrors the completeness result of \cite{poor}; where symplectic geometry is secretly hiding behind the normal form which they use to prove completeness.
