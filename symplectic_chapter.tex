...
%\end{lemma}
%\begin{definition}
%A {\bf symplectic basis} for a symplectic vector space is a basis $\{b_0,\ldots, b_{2n-1}\}$ such that for all $b_i$ and $b_j$, with $i\neq j$, $\omega(b_i,b_j)=0$.
%\end{definition}
%Every symplectic vector space admits a (non-unique) symplectic basis.   Moreover, every 
...
The following  categories of linear isotropic/coisotropic/Lagrangian relations generalizes symplectomorphisms in a way that allows nondeterministic evolution:
\begin{definition}
...
Where recall that the antipode is given by:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
\end{tikzpicture}
$$
This also allows us to capture (linear) resistors as Lagrangian relations.  Given a linear resistor with resistance $r\in \R^{+}$, by Kirchhoff's current  law the incoming current is equal to the outgoing current; and by By Ohm's law, the outgoing potential is equal to the current multiplied by $r$ plus the incoming potential:
$$
\left\llbracket
...
\end{tikzpicture}
$$
In \cite{passive} they augment this semantics with time-dependent components.  The exposition of which necessitates  the following construction:
\begin{definition}
...
To show this, we  first recall two results of Gross, relating the stabilizer formalism to symplectic geometry.  We reproduce the  proofs here because of their importance.  First we need the following convention to represent elements of the Heinsenberg-Weyl group:
\begin{definition}
...
\begin{lemma}[Weil representation {\cite[Theorem 3]{gross}}]
...
\begingroup\allowdisplaybreaks
\begin{lemma}[{\cite[Lemma 8]{gross}}]
...
%We seek to recast these results in a more compositional light.  This is because we will prove that for odd prime $p$, $\Aff\Lag\Rel_{\F_p}$ is isomorphic to quopit stabilizer circuits, modulo scalars.  Let us give the latter category a name:
\begin{definition}
...
Later, this was formalized in terms of symplectic geometry \cite{spekkens2016quasi}. %, %where pure states in the toy model are regarded as affine Lagrangian subspaces over $\F_2$ and the reversible evolution is given by affine symplectomorphisms \cite{spekkens2016quasi}.% The knowledge balance principle corresponding to the preservation of the symplectic form . 
...
\begin{theorem}[The symplectic ZX-calculus]\
...
\end{align*}
The scaling gate, which is a derived generator in this setting is interpreted as:
$$
\left\llbracket \
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
Up until now, we have also neglected to relate the symplectic picture to the \dag-structure of $\FHilb$.  This will be instrumental in the following section:
\begin{definition}
\label{def:conj}
...
\end{proof}
\begin{remark}
  We restate the interpretation of passive electrical circuits in terms of phased spiders for affine Lagrangian relations. An idealized junction, a resistor with resistance $r$, a capacitor with capacitance $c$ and an inductor with inductance $\ell$ are interpreted as:
...
$$
 ...
 In \cite{network}, the relational semantics of electrical circuits is broadened even further to capture current and voltage sources as affine Lagrangian relations over $\R(s)$.
...
\section{Affine coisotropic relations and mixed stabilizer circuits}
\label{sec:coisotrel}
\label{sec:coisot}
...
\begin{theorem}[Symplectic Stinespring dilation]
\label{thm:symstine}
%Every isotropic subspace of $k^{2n}$ of dimension $n-m$ is the kernel of a Lagrangian coisometry $n\to m$.
...
%
%This follows because composing $W$ with the cozero maps on the last wire of the $X$ and $Z$ blocks picks out the rows where the last $m$ entries of the  $Z$ and $X$ blocks are both required to be $0$; that is, those of the generator matrix of $V'$.  Therefore, every isometry is the kernel of a Lagrangian relation.  Moreover, 
%
%
%$$
%\begin{tikzpicture}
%	\begin{pgfonlayer}{nodelayer}
%		\node [style=map] (36) at (226.5, 2.25) {$W$};
%		\node [style=none] (37) at (227.5, 4.25) {};
%		\node [style=none] (38) at (226.75, 4.25) {};
%		\node [style=map] (39) at (224.75, 2.25) {$\bar W$};
%		\node [style=Z] (40) at (226.25, 3.75) {};
%		\node [style=X] (41) at (224.5, 3.75) {};
%		\node [style=none] (42) at (227.5, 4.25) {};
%		\node [style=none] (43) at (226.75, 4.25) {};
%		\node [style=none] (44) at (225.75, 4.25) {};
%		\node [style=none] (45) at (225, 4.25) {};
%	\end{pgfonlayer}
%	\begin{pgfonlayer}{edgelayer}
%		\draw [in=45, out=-90] (37.center) to (36);
%		\draw [in=-90, out=120] (36) to (38.center);
%		\draw [in=-15, out=75, looseness=1.25] (36) to (40);
%		\draw [in=-135, out=150, looseness=1.50] (39) to (41);
%		\draw [in=-90, out=105] (39) to (45.center);
%		\draw [in=-90, out=30, looseness=0.75] (39) to (44.center);
%		\draw [in=-165, out=75] (39) to (40);
%		\draw [in=-15, out=150, looseness=1.25] (36) to (41);
%	\end{pgfonlayer}
%\end{tikzpicture}
%=
%\begin{tikzpicture}
%	\begin{pgfonlayer}{nodelayer}
%		\node [style=X] (56) at (229.5, 3.75) {};
%		\node [style=Z] (57) at (231.25, 3.75) {};
%		\node [style=none] (58) at (229, 4.5) {};
%		\node [style=none] (59) at (230, 4.5) {};
%		\node [style=none] (60) at (230.75, 4.5) {};
%		\node [style=none] (61) at (231.75, 4.5) {};
%	\end{pgfonlayer}
%	\begin{pgfonlayer}{edgelayer}
%		\draw [in=-90, out=30] (57) to (61.center);
%		\draw [in=-90, out=150] (57) to (60.center);
%		\draw [in=-90, out=30] (56) to (59.center);
%		\draw [in=-90, out=150] (56) to (58.center);
%	\end{pgfonlayer}
%\end{tikzpicture}
%$$
%
%
%By applying the inverse controlled-$\mathcal X$ gate and inverse Fourier transform to $V'$ which we first applied, then there is a Lagrangian subspace $W'\subseteq k^{2(n+m)}$:
%
%$$
%\begin{tikzpicture}
%	\begin{pgfonlayer}{nodelayer}
...
%	\begin{pgfonlayer}{nodelayer}
%		\node [style=X] (56) at (229.5, 3.75) {};
%		\node [style=Z] (57) at (230.25, 3.75) {};
%		\node [style=none] (58) at (229, 4.5) {};
%		\node [style=none] (59) at (230.25, 4.5) {};
%		\node [style=none] (60) at (229.5, 4.5) {};
%		\node [style=none] (61) at (230.75, 4.5) {};
%	\end{pgfonlayer}
%	\begin{pgfonlayer}{edgelayer}
%		\draw [in=-90, out=30] (57) to (61.center);
%		\draw [in=-90, out=150] (57) to (60.center);
...
%\begin{tikzpicture}
%	\begin{pgfonlayer}{nodelayer}
%		\node [style=map] (17) at (223.25, -0.5) {$E$};
%		\node [style=Z] (20) at (221.75, 0.5) {};
%		\node [style=X] (21) at (222.5, 0.5) {};
%		\node [style=none] (26) at (223, 1) {};
%		\node [style=none] (27) at (223.5, 1) {};
%		\node [style=none] (28) at (221.5, -1) {};
%		\node [style=none] (29) at (222.25, -1) {};
%		\node [style=none] (30) at (221.5, -1.25) {$n$};
%		\node [style=none] (32) at (222.25, -1.25) {$n$};
%		\node [style=none] (33) at (223, 1.25) {$1$};
%		\node [style=none] (34) at (223.5, 1.25) {$1$};
%	\end{pgfonlayer}
%	\begin{pgfonlayer}{edgelayer}
%		\draw [in=-15, out=90, looseness=1.25] (17) to (21);
%		\draw [in=-30, out=135, looseness=0.75] (17) to (20);
%		\draw [in=90, out=-105] (20) to (28.center);
%		\draw [in=255, out=90] (29.center) to (21);
%		\draw [in=270, out=120] (17) to (26.center);
%		\draw [in=-90, out=60] (17) to (27.center);
%	\end{pgfonlayer}
%\end{tikzpicture}
%=
%\begin{tikzpicture}
%	\begin{pgfonlayer}{nodelayer}
%		\node [style=map] (17) at (223.25, -0.5) {$W'$};
%		\node [style=Z] (20) at (221.75, 0.5) {};
%		\node [style=X] (21) at (222.5, 0.5) {};
%		\node [style=none] (26) at (223, 1) {};
%		\node [style=none] (27) at (223.5, 1) {};
...
%
%Suppose that we have an isotropic subspace $V$ of $(k^{2n},\omega)$ with dimension $n-m$; by induction, $V$ can be purified to a  Lagrangian subspace of $k^{2(n+m)}$.
%\end{proof}
...
%\begin{remark}
%\label{rem:ranknull}
%As observed in  \cite[\S 3.4.4]{guillemin2013semi}, we have a symplectic version of the rank-nullity theorem for Lagrangian relations:
%$$
%\begin{tikzpicture}
%	\begin{pgfonlayer}{nodelayer}
...
%\begin{definition}
%Given some state $\hat f:0\to n$ in $\CPM(\Aff\Lag\Rel_k)$, the (mixed)  {\bf symplectic stabilizer group} of $f$ is the subgroup of $S_{\hat f} \subseteq (\Z/p\Z)^{2n}$  defined by  $(z,x) \in S_{\hat f}$ if and only if:
%$$
%\begin{tikzpicture}
%	\begin{pgfonlayer}{nodelayer}
%		\node [style=none] (0) at (2.25, 3.25) {};
%		\node [style=none] (1) at (3.75, 3.25) {};
%		\node [style=map] (2) at (3, 2.25) {$\hat f$};
%		\node [style=map] (3) at (3.75, 3.25) {$W(z,x)$};
...
%%\begin{tikzpicture}
%%	\begin{pgfonlayer}{nodelayer}
%%		\node [style=map] (0) at (4, 3.25) {$W(z,x)$};
%%		\node [style=none] (1) at (2.5, 3.25) {};
%%		\node [style=Z] (2) at (3, 2.75) {};
%%		\node [style=map] (3) at (3.75, 2) {$f$};
%%		\node [style=map] (4) at (2.25, 2) {$\bar f$};
...
%		\node [style=map] (21) at (11, 2.25) {$\hat f$};
%	\end{pgfonlayer}
%	\begin{pgfonlayer}{edgelayer}
...
%%
%%\begin{lemma}
%%Given some state $\hat f:0\to n$ in $\CPM(\Aff\Lag\Rel_k)$  with an affine Lagrangian purification
%%$(f:0\to m+n,m)$, then:
%%$$
%%S_{\hat f}=
%%\begin{tikzpicture}
%%	\begin{pgfonlayer}{nodelayer}
%%		\node [style=X] (75) at (235.25, 0.5) {};
%%		\node [style=X] (76) at (234.35, 0.5) {};
%%		\node [style=map] (77) at (235, -0.5) {$f$};
%%		\node [style=none] (78) at (234.8, 1) {};
%%		\node [style=none] (79) at (235.65, 1) {};
%%	\end{pgfonlayer}
%%	\begin{pgfonlayer}{edgelayer}
%%		\draw [bend right, looseness=0.75] (77) to (79.center);
%%		\draw [bend right=15] (77) to (75);
%%		\draw [bend left=15] (77) to (78.center);
%%		\draw [bend left, looseness=0.75] (77) to (76);
%%	\end{pgfonlayer}
%%\end{tikzpicture}=:T
%%$$
%%\end{lemma}
%%\begin{proof}
%%Any of $T$ is clearly in $S_{\hat f}$.  Conversely take some $(z,x) \in S_{\hat f}$.  If 
%%\end{proof}
%
%
%\begin{lemma}
%Given some state $\hat f:0\to n$ in $\CPM(\Aff\Lag\Rel_k)$, $S_{\hat f}$ is an affine isotropic subspace of $\F_2^n$.
%\end{lemma}
%
%
...
%\end{proposition}
%\begin{proof}
%Take two states $\hat f:0\to n$ and $\hat g:0\to n$ in  $\CPM(\Aff\Lag\Rel_k)$ with the same stabilizer group $S$.  Suppose that their stabili
%Chose  affine Lagrangian purifications:
%$$
...
%\hat g:0\to m
%$$ 
%Fix bases for $f$ and $g$
%$$
%f={\sf span}
%\left[\begin{array}{cc|cc}
%Z_{f,0} & Z_{f,1} & X_{f,0} & X_{f,1}
%\end{array}\right]+A_f
%\ , \hspace*{.2cm}
%g={\sf span}
%\left[\begin{array}{cc|cc}
%Z_{g,0} & Z_{g,1} & X_{g,0} & X_{g,1}
%\end{array}\right]+A_g
%$$
%
%
...
%We can construct a unitary  $u:m\to m$ which performs Gaussian elimination on the first $m$ wires of $f$ so that:
% %and isometry $v:\ell\to m$, which perform Gaussian elimination on the first $$ $f$ and $g$ so that:
%$$ 
%\left[\begin{array}{cc|cc}
%Z_{f,0}  & Z_{f,1} & X_{f,0} & X_{f,1}
%\end{array}\right]+A_f
%\xmapsto{u\otimes 1_n}
%\left[\begin{array}{cc|cc}
% I_m & Z_{f,2} & 0             & X_{f,2}\\
%0      & Z_{f,3}& 0              & X_{f,3}
%\end{array}\right]+
...
%\end{array}\right]
%$$
%And similarly, there is an isometry $v:\ell\to m$ acting on the first $\ell$ wires of $f$ so that:
%$$
%\left[\begin{array}{cc|cc}
%Z_{g,0} & Z_{g,1} & X_{g,0} & X_{g,1}
%\end{array}\right]+A_g
%\xmapsto{v\otimes 1_n}
%\left[\begin{array}{cc|cc}
% I_m & Z_{g,2} & 0             & X_{g,2}\\
%0      & Z_{g,3}& 0              & X_{g,3}
%\end{array}\right]
...
%$$
%%
%%Then $f'=(u\otimes 1_n) f$ is a purification of $\hat f$ and $g'=(v\otimes 1_n) g$ is a purification of $\hat g$.
%%We see that the stabilizer group of $f'$ is generated by  $(z',z,0,x)$ where  $(z,x)$ is a stabilizer of $\hat f$, $\forall z' \in \F_p^m$.  
%%Similarly, the stabilizer group of $g'$ is generated by  $(z',z,0,x)$ where  $(z,x)$ is a stabilizer of $\hat g$, $\forall z' \in \F_p^m$.  But the stabilizer groups of $\hat f$ and $\hat g$ are the same; therefore, the stabilizer groups of $f'$ and $g'$ are the same, so that $f'=g'$.
%%
%
%%Take a stabilizer $(z,x)$ of $\hat f$ (or equivalently $\hat g$).  If $(z,x) \in {\sf span}()$
%
%
%
%Every element in ${\sf span} [Z_{f,3}|X_{f,3}]+f_{A}'$ is a 
%
%
%
%Therefore:
...
%Consider the identity on objects map $\CPM(\Aff\Lag\Rel_k) \to \Aff\Co\Isot\Rel_k$, sending:
%$$
%\begin{tikzpicture}
%	\begin{pgfonlayer}{nodelayer}
%		\node [style=map] (0) at (3.75, 2.25) {$f$};
...
%\end{tikzpicture}
%$$
%This is full by Theorem \ref{thm:symstine} and well defined and faithful by Proposition \ref{prop:uniqueness}.
%
...
%%Then there is an affine symplectomorphism  $u:m\to m$  and isometry $v:\ell\to m$, which performs Gaussian elimination on the ancillary systems of $f$ and $g$ so that:
%%$$
%%\begin{tikzpicture}
%%	\begin{pgfonlayer}{nodelayer}
...
%%\end{tikzpicture}
%%$$
%%So that, as before, they are both purifications of $\hat f = \hat g$.
%\end{proof}
...
%We know that $\Aff\Lag\Rel_k$ is generated by the two spiders and their phase groups.  Take any such generator $g$. Then $(z,x)$ is a stabilizer for $g$ if and only if $(-z,g)$ is a stabilizer for $\bar g$.
...
Mixed stabilizer states are also known as {\bf stabilizer codes} for reasons that will become clear in Section \ref{sec:qec}.
...
%There is a symplectic analogue to Stinespring dilation, which we will use in Section \ref{sec:qec} in our discussion of error correction:
...
In order to add measurement and state preparation in the symplectic setting we inspect the structure of the $Z$ and $X$ projectors in the symplectic setting:
\begin{definition}
...
%\end{corollary}
%The maps are interpreted as the epistemic states and their symplectic complements as the ontic states. When part of the epistemic state is discarded, then then the dimension can increase; meaning that the dimension of the ontic state decreases. This will make more sense when we talk about measurement in Remark \ref{rem:wig}.
%
...
$$
\end{remark}
For this reason, we can prove the correctness of the qupit quantum teleportation algorithm discussed previously in Protocol \ref{proto:tele} using only spider fusion:
\begin{example}
...
\ \right\rrbracket
$$
On the other hand the dual coloured prop $\Aff\Isot\Rel_{\R(x)}^M$ gives a semantics for all of the uncontrolled components we have discussed so far as well as voltmeters and ammeters:
$$
\left\llbracket\
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
The graphical algebra approach to error correction which we employ in this section strictly generalizes that of \cite{grok}; where they use linear relations over $\F_2$ as the semantics for CSS-codes. Although we give much more elementary examples.
...
Recalling Corollary \ref{cor:affsymstine}, fix a unitary dilation $U$ of $S$:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=map] (249) at (105.1, 3.25) {$S$};
...
\ \right\rrbracket
$$
The induced isometry, called the {\bf encoder}, embeds $k$ logical quopits into $n$ physical quopits:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=map] (249) at (105.1, 3.25) {$U$};
...
To detect the error, Bob applies the non-destructive measurement with respect to $H(U)(1_{k}\otimes{\cal Z}^{\otimes (n-k)})H(U)^\dag$, which we draw in our calculus as follows:
...
$$
 ...
 Because stabilizers preserve Weyl-operators under conjugation, in the symplectic picture, we have $W((a,b),(c,d)) = U;W(e);U^\dag$.  Therefore:
\begin{align*}
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
%Bob must chose a strategy for how to correct errors.  %Given a s
%
%
%applies classically controlled Weyl-operators using the information which was measured in the syndrome.
...
If $f$ is an affine transformation, then there is a classically controlled operation
...
% To the knowledge of the author, this question has not yet been addressed, presumably, because classical processing and stabilizer codes are usually presented on different footings, as opposed to both being regarded themeselves as subspaces.  Does this correspond to  arbitrary convex combinations of qupit stabilizer states?
...
%The normal form is effectively a special case of the AP-normal form of \cite{poor}, except over positive division semirings, rather than over $\F_p$.
...
%
%Finally, in our analysis of stabilizer codes and error correction, we have neglected to describe the effect interpreting a stabilizer code as a doubled biaffine ZX-diagram on code distance. In the literature, there is a similar doubling which preserves code distance involving Majorana operators.  In \cite{majorcodes} they describe a procedure which takes ``Majorana fermion codes'' on $2n$-physical Majorana modes encoding $m$-logical qubits with code distance $d$ to a $[2n,2k,d]$ qubit CSS-code.
%I can not say for certain, because of the language barrier between mathematics and physics, but it appears that Majorana fermion codes correspond to isotropic subspaces with respect to the {\em hyperbolic} quadratic form  on $\R^{2n}$:
%$$
%\begin{bmatrix}
%0 & I\\
%I & 0
%\end{bmatrix}
%$$
%It would be interesting to investigate if there is a connection between these two notions of isotropic subspaces.
