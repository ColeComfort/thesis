...
For example given another matrix $B:\ell^2((\Z/d\Z)^m)\to \ell^2((\Z/d\Z)^\ell)$ with entries $b_{k',j'}$:
...
In quantum mechanics, measurement on a system $\mathcal H$ is performed with respect to an orthonormal basis.  Fix an orthonormal basis $B=\{ |b_j\rangle \}_{j \in \mathcal J}$ of $\mathcal H$.   Given a quantum state state $|\phi\rangle$ then
$$
\sum_{j\in \mathcal J}
 |\langle b_j|\psi \rangle|^2
=1
$$ 
Therefore this gives a probability distribution over $\mathcal J$.  That is, the probability of measuring basis vector $| b_j\rangle $ is given by $| \langle b_j | \psi \rangle |^2$. 
This probabilistic interpretation of quantum measurement is called the {\bf Born rule}, although it is usually stated slightly differently.
This begs the question:  what does the Born rule have to do with the $\CPM$ construction?
...
We could have done the same with $Z$-stabilizers, where the $\mathcal Z$-gate is given by the Fourier transform of the $\mathcal X$-gate:
$$
{\cal Z}
:=
{\cal F}
...
Notice how the $Z$-basis is generated by $\mathcal{X}^n|0\rangle $ and the $X$-basis is generated by $\mathcal{F} \mathcal{X}^n |0\rangle =\mathcal{Z}^n \mathcal{F} |0\rangle$.
