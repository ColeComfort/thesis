...
\end{definition}
The reason why stabilizer states are so nice is because we can choose to work with their very well-behaved stabilizer groups:
\begin{lemma}
...
Later, two complete axiomatizations for the qubit ZX-calculus followed, where all phases are included.  This was done independently by Ng and Wang as well as Jeandel et al.  \cite{zxcompleteb,zxcompletea}. These presentations of the qubit ZX-calculus are universal on the nose, so that they both are presentations for the full subcategory of $\Mat_\C$ whose objects are powers of $2$.
...
Recently, in a universal set of generators has been proposed for the qudit ZH-calculus in the Masters thesis of Roy \cite{roy}, but no completeness theorem exists so far.
