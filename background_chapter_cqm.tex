...
String diagrams have been used in quantum theory for quite some time; in particular, as early as the work of Penrose \cite{penrosei}.  In such settings, string diagrams have been used as (often heuristic) tools for calculation. The more recent programme of ``categorical quantum mechanics,'' following the seminal paper of Abramsky and Coecke \cite{abramsky} has endeavoured to reformulate finite dimensional quantum mechanics using category theory.  In this setting,  string diagrams are formal mathematical objects: allowing certain essential features to be abstracted away from the usual setting of finite dimensional Hilbert spaces.

In this section, we  review this formalism as well as recent developments which are relevant to this thesis.  A more in depth mathematical introduction can be found in the book of Heunen and Vicary \cite{heunen}, with a more broadly accessible and applied introduction being found in the book of Coecke and Kissinger \cite{pqp}.  For a more traditional approach to quantum computing see the book of Nielsen and Chuang \cite{nielsen}.
...
\section{Quantum states and unitary evolution}
...
The elements of an orthonormal basis of $\mathcal H$ indexed by a set $\mathcal J$ are drawn in ``ket notation'' by $|b_{j}\rangle$, for all $j \in \mathcal J$.
...
This notation allows us to succinctly represent matrices.  For example, a complex matrix from $d^n$ to $d^m$ regarded as an operator $A:\ell^2((\Z/d\Z)^n)\to \ell^2((\Z/d\Z)^m)$ with entries $a_{j,k}$ is denoted as follows:
$$
...
\right)
$$
is equal to the identity if and only if the basis is orthonormal:
$$
\sum_{j\in \mathcal J}
\left(\
\begin{tikzpicture}
...
Selinger gives a construction to produce categories of quantum channels from general \dag-compact closed categories \cite{cpm}. When applied to $\FHilb$, this construction adds discarding behavior to quantum systems.  This will provide the necessary machinery to model measurement. 
...
\begin{definition}[CPM construction]
\label{def:cpm}
...
\begin{definition}
\label{def:idem}
Given a category $\X$ and a class of idempotents $\mathcal I$, the {\bf Karoubi envelope} ${\sf Split}_{\mathcal I}(\X)$ of $\X$ at $\mathcal{I}$,  is the category with:
\begin{description}
...
\end{description}
In particular, when $\mathcal{I}$ contains all idempotents in $\X$, call  ${\sf Split}(\X):={\sf Split}_{\mathcal I}(\X)$  the Karoubi envelope of $\X$.
...
\end{definition}
Selinger shows that splitting projectors in $\CPM(\FHilb)$ yields a category where the the split projectors can be interpreted as classical types  \cite{idempotent}:
\begin{remark}
\label{rem:stoch}
...
Take $\mathcal{H}=\mathcal{H}'$.  Up to global phase, the subobject $(\mathcal{H},e_B)$ can be identified with the the Hilbert space $\mathcal{H}$; where state preparation with respect to $\xcirc$ and nondestructive measurement with respect to $\zcirc$ correspond to the following maps:
$$ 
...
There is a graphical calculus for this two-coloured prop of classical and quantum types.  
...
\ \right\rrbracket
$$
The conjugation of pure spiders by state preparation and measurement maps creates {\bf thick-thin spiders} following Coecke and Kissinger \cite{pqp} (which they call bastard spiders).  Thick-thin spiders are drawn with a thin border to distinguish them from pure spiders, however, their legs can be either thick or thin.  For example:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
This notation allows us to succinctly state a special notion of purification in $\CPM(\FHilb)$:
...
\end{proposition}
The proof is relatively involved so we will omit it.  See Stinesprings paper  for the original statement\cite{Stinespring1955}, and  Coecke and Kissinger's book for the graphical version which we use \cite[Corollary 6.63]{pqp}.  Essentially, the significance  of this result is that quantum processes can always be produced by first preparing a state, then applying quantum evolution and then discarding part of the state.
...
Given two strongly complementary observables, we can construct the quantum teleportation protocol  (originally discovered for qubits by Bennett et al. \cite{teleportation}).
The abstract description of quantum teleportation in terms \dag-compact closed categories was first introduced by Abramsky and Coecke \cite{abramsky}; however, we present a qudit generalization of the one using thick-thin spiders found in Coecke and Kissinger's book \cite[Page 706]{pqp}:
...
So that if Alice prepares a quantum state $|\phi\rangle$, then Bob receives it:
...
\end{tikzpicture}
$$
In other terms, following Carette, the $Z$ and $X$-spiders are {\bf flexsymmetric}   \cite[\S 5]{flexsymmetric}.
...
\end{definition}
The {\bf scalable ZX-calculus} (coined by Carette et al. \cite{szx}) refers to string diagrams for the strictification of the ZX-calculus (ie, proof nets for the ZX-calculus). Frobenius algebras on wires of composite dimension are denoted as follows:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
\end{tikzpicture}
$$
As we use string diagrams for nonstrict monoidal categories extensively throughout this thesis, we won't declare when we are using scalable ZX-notation; it will just be the default setting in which we work.
...
$$
\end{definition}
The $X$ stabilizers characterize the phase-free ZX-calculus: \footnote{This has been known for quite some time to both the categorical concurrency and quantum communities in the qubit case, see for example in the thesis of Zanasi \cite[Page 8]{ih}.}
\begin{lemma}
...
$$
S\mapsto \dfrac{1}{|S|} \sum_{(x_1,\cdots, x_n) \in S} | x_1, \cdots x_n \rangle\langle x_1, \cdots, x_n  |
$$
...
$$
So one can ask if the fragment of the odd prime qudit ZX-calculus with these $\cal X$-gate phases has a similar relational semantics to the phase-free ZX-calculus.  The answer is yes, and this result is not contained in the literature to the knowledge of the author:
\begin{lemma}
...
The $n$-quopit Weyl operators form the {\bf Heisenberg-Weyl group} $\mathcal{P}_d^n$ under matrix multiplication and the Hermitian adjoint.  The $n$-qubit  Heisenberg-Weyl group is generated by $n$-qubit Weyl operators in addition to the scalar $i$. 
\end{definition}
Note that the qubit  Heisenberg-Weyl group is often called the Pauli group and its elements are called Pauli operators. 

Weyl operators have the following property (see \cite[\S 10.3.1]{nielsen}):
\begin{lemma}
\label{lem:ueb}
$n$-qupit Weyl operators are an orthonormal basis for the finite dimensional  Hilbert space of operators
$$
\FHilb(\ell^2(\F_p^n), \ell^2(\F_p^n))
\cong \ell^2(\F_p^n)^*\otimes \ell^2(\F_p^n)
$$
...
An obvious choice of operator basis would have been:
$$
\{ |a_0, \cdots, a_{n-1} \rangle \langle  b_0,\cdots, b_{n-1} | \ | \ a_0,\cdots, a_{n-1},b_0,\cdots, b_{n-1} \in \F_p \}
$$
...
$$\{ \mathcal{X}^n|0\rangle \}_{n\in \F_p}
\ , \hspace{,5cm} \{\mathcal{F}|n\rangle=\mathcal{F} \mathcal{X}^n |0\rangle =\mathcal{Z}^n \mathcal{F} |0\rangle\}_{n\in \F_p}$$
...
It was first introduced by Gottesman   \cite{gottesman} and later generalized to qudits in various ways.  We follow the qudit generalization of Gottesman \cite{gota}:
\begin{definition}
An $n$-qupit {\bf Clifford operator} $U$ is a unitary on $\ell^2(\F_p^n)$ that preserves the Heisenberg-Weyl group, so that $U {\cal P}_p^{ n} U^\dag = {\cal P}_p^{ n}$.  The $n$-qupit Clifford operators form the $n$-qupit {\bf Clifford group } under matrix multiplication and the Hermitian adjoint.  The qupit Clifford groupoid (or full qupit Clifford group) is the prop where the maps $n\to n$ are qupit Clifford operators.


An $n$-qupit (pure) {\bf stabilizer state} is a state $ U |0\rangle^{\otimes n}$ for an $n$-qudit Clifford $U$.  


Given any $n$-qupit pure stabilizer state $|\psi \rangle$,  the {\bf stabilizer group} of $|\psi \rangle$   is the subgroup of ${\cal S}_{|\psi\rangle} \subset {\cal P}_p^{ n}$  whose elements  stabilize $|\psi\rangle$.
\end{definition}
The reason why stabilizer states are so nice is because  their representation in terms of stabilizer groups is very convenient, owing to the highly symmetric structure of the Heisenberg-Weyl group.  
\begin{lemma}
Up to global phase, stabilizer states are in bijective correspondence their stabilizer groups, and stabilizer groups are in bijection with maximal Abelian subgroups of $ {\cal P}_p^{ n}$.
\end{lemma}
\begin{definition}
The prop of qupit {\bf stabilizer circuits} is generated by qupit Clifford operators as well as the state $|0\rangle$ and effect $\langle 0|$.
\end{definition}
...
Up to nonzero scalars, the qupit Clifford groupoid is generated by the Fourier transform $\mathcal F$, the $\mathcal X$-gate, the  phase-shift gate $\mathcal S$, controlled-$\mathcal X$ gate $\mathcal{C}_{\mathcal X}$, and scaling gates $\mathcal{M}_a$ for every $a \in \F_p^*$ where:
$$
...
\begin{definition}
\label{def:biaffine}
Given any prime $p$, the {\bf biaffine} fragment of the qupit stabilizer ZX-calculus is generated by $Z$ and $X$ spiders with phases in the group $\Z/p\Z$ where:
$$
\Z/p\Z \to [0,1)^p; \ n \mapsto  \prod_{j=0}^{p-1} (n j)/(2p)
$$
\end{definition}
Therefore, anytime we make a statement about quopit stabilizer circuits that doesn't refer to the Fourier transform or phase-shift gates, it also applies to biaffine qupit stabilizer circuits.

Concretely, the biaffine fragment of the qubit stabilizer ZX-calculus corresponds to the sub-fragment of the real fragment of qubit stabilizer ZX-calculus, where moreover, each stabilizer of a given state is proportional either to a string of exclusively X-stabilizers or exclusively Z-stabilizers.

The qubit  and qutrit  stabilizer ZX-calculi both have complete presentations \cite{backensstab,backens2015} and \cite{qutrit}.  However, during the process of writing this thesis, Booth and Carette gave a complete  axiomatization for the quopit stabilizer ZX-calculus \cite{booth}, followed shortly by an even simpler presentation by Po\'{o}r et al. \cite{poor}.  Note that the interpretations of the generators in these papers differ from ours slightly for technical reasons:  they designed their presentations to be flexsymmetric.
...
The qubit case seems to be folklore; Campbell et al.'s reference which we have cited only proves it for quopits.  Surely this also holds for all dimensions, but the group theory becomes harder.  It is dense in an efficient way, by the Solvay-Kiataev theorem.  This is a technical result, first proved initially for qubits; the history again is kind of nuanced, it was first unofficially announced by Solvay, and published by Kitaev \cite{Kitaev1997}.  This result was  later generalized to all qudits by Dawson et Nielsen \cite[\S 5]{Dawson2006}.

Therefore, by adding any other phases to qupit stabilizer circuits is maximally expressive.  Jeandel et al. constructed the first approximately universal axiomatization of the qubit ZX-calculus in this way \cite{Jeandel}.

Later, two complete axiomatizations for the qubit ZX-calculus followed, where all phases are included.  These two axiomatizations were proven independently.  Ng and Wang gave one axiomatization and Jeandel et al.  gave the other; almost at the same time \cite{zxcompleteb,zxcompletea}. These presentations of the qubit ZX-calculus are universal on the nose, so that they both are presentations for the full subcategory of $\Mat_\C$ whose objects are powers of $2$.

\section{The ZH-calculus}
...
To accommodate for this, {\bf $H$-boxes} were devised in the qubit case by Backens and Kissinger \cite{zh}:
\begin{definition}
\label{def:zh}
...
The $H$-box with label $-1$ and one input and one output is equal to $\sqrt{2}\mathcal{F}$.
...
$$
\end{definition}
In analogy to the ZX-calculus, a {\bf fragment of the ZH-calculus} is presented by unphased $Z$ and $X$ spiders, in addition to  $H$-boxes labelled by a semiring $S$ and an interpretation into $\FHilb$. The interpretation must send the $Z$ and $X$ spiders to the $Z$ and $X$ spiders in $\Hilb$ in the same way as for the phase-free ZX-calculus. We also ask that there is a semiring homomorphism $f:S\to \C$ such that 
$$
\left\llbracket
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
The full qubit ZH-calculus was proved to be complete and universal for all qubit complex matrices in the original paper of Backens and Kissinger \cite{zh}.  

Recall that an H-box with no label is the same as an H-box which is labelled by $-1$.  Therefore an axiomatization for the phase-free ZH-calculus must be compatible with the semiring homomorphism $\Z\to\C$.
Completeness for the phase-free ZH-calculus up to a scalar factor was proven by van de Wetering and Wolffs; giving a presentation for qubit matrices over $\Z[1/\sqrt{2}]$ \cite{zhpi}.
In their paper, they also prove that the  phase-free ZH-calculus is approximately universal for quantum circuits.
...
Recently, in a universal set of generators has been proposed for the qudit ZH-calculus in the Ms.C. thesis of Roy \cite{roy}, but no completeness theorem exists so far.
