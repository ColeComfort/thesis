...
Given two strongly complementary observables, we can construct the quantum teleportation protocol  (originally discovered for qubits by \cite{teleporation?}).
...
This explains why we called the final stage of the quantum teleportation phase correction.  When Bob prepares the quantum state $|n\rangle$ using the white bastard spider
and fuses to the quantum channel with the pure grey spider, he is applying the gate $\mathcal{X}^n$.  Dually, when he prepares the state $\mathcal{F}|m\rangle$ and fuses it to the channel with the pure white spider he is applying the gate $\mathcal{Z}^m$.  
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		...
		\node [style=map, thick] (240) at (223.5, -1) {$\mathcal{Z}^m$};
		\node [style=map, thick] (25-) at (223.5, 2) {$\mathcal{X}^n$};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw [thick] (28.center) to (22.center) to (23.center)  to (29.center);
		\draw [thick, in=-165, out=90, looseness=0.75] (25.center) to (23.center);
...
Where the second last equation follows from the fact that $\mathcal{X}^n$ is a phase for the grey spider and $\mathcal{Z}^n$ is a phase for the white spider, followed by spider fusion
