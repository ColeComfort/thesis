...
Given a pure quantum state in $\FHilb$, its doubled version is a quantum state in $\CM(\FHilb)$.
...
This representation of linear subspaces, and consequently, matrices within the ZX-calculus has recently been exploited using proof net notation: ie the scalable ZX-calculus \cite{szxi,szxii}.
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		...
		\node [style=scalarop] (4) at (3, -2.5) {$a$};
		\node [style=scalar] (5) at (4, -2.5) {$a$};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw (0.center) to (4);
		\draw (4) to (1.center);
