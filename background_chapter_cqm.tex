...
In this section, we shall briefly review this formalism as well as recent developments which are relevant to this thesis.  A more in depth mathematical introduction can be found in \cite{heunen}, with a more broadly accessible and applied introduction being found in \cite{pqp}.
...
As a matter of notation we will colour the special commutative \dag-Frobenius algebra $\zcirc$.  We will colour the commutative \dag-Frobenius algebra for other, orthogonal bases $\xcirc$.  Recall the two variations of the normal form for special and non-special commutative \dag-Frobenius algebras:
...
In the doubled picture, the composition of equivalence classes is  composition in $\X$:
...
The $\CPM$ construction and special commutative $\dag$-Frobenius algebras allow us to perform quantum measurement using only string diagrms.  Some mathematical machinery is needed first:
...
The thin border indicates that the state has been measured in the basis $\zcirc$, and is no longer pure and is a stochastic mixture of the basis element of $B$.
...
In general, we will draw thick borders on arbitrary pure maps $U$; and thick borders on mixed maps $V$ as follows:
...
For all special \dag-commutative Frobenius algebras $\zcirc$.
\end{proposition}
The proof is relatively involved so I will omit it.  See \cite{???} for the original statement, and \cite[????]{pqp} for the graphical proof using bastard spiders.  Essentially, this significance  of this result is that quantum processes can always be produced by first preparing a state, then applying quantum evolution and then discarding part of the state.
...
Take \dag-commutative Frobenius algebras $\zcirc$ and $\xcirc$. They are   {\bf strongly complementary} when the the bialgebras are Hopf algebras whose antipode is equivalently any of the following maps:
$$
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
...
Given two strongly complementary obsevables, we can construct the quantum teleportation protocol  (originally discovered for qubits by \cite{teleporation?}).
The abstract description in terms of quantum teleportation in terms \dag-compact closed categories was first introduced in \cite{abramksy}; however, we present a qudit generalization of the one found in \cite[Page 706]{pqp}:
...
\item
 Bob uses the two classical dits two perform phase-correction operations using the complementary basis to his half of the Bell state.
...
The $\mathcal Z$-gate is the Fourier transform of the  $\mathcal X$-gate:
$$
{\cal Z}
:=
{\cal F}
...
So we could have instead characterized the phase-free+$Z$ gate fragment of the ZX-calclus in terms of affine relations, via the $Z$-stabilizers.





Notice how the $Z$-basis is generated by 
$$\{ \mathcal{X}^n|0\rangle \}_{n\in \Z/d\Z}$$
 and the $X$-basis is generated by 
$$\{\mathcal{F}|n\rangle=\mathcal{F} \mathcal{X}^n |0\rangle =\mathcal{Z}^n \mathcal{F} |0\rangle\}_{n\in \Z/d\Z}$$


 This explains why we called the final stage of the quantum teleportation phase correction.  When Bob prepares the quantum state $|n\rangle$ using the white bastard spider and fuses to the quantum channel with the pure grey spider, he is applying the gate $\mathcal{X}^n$.  Dually, when he prepares the state $\mathcal{F}|m\rangle$ and fuses it to the channel with the pure white spider he is applying the gate $\mathcal{Z}^m$.  
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		...
		\node [style=map] (26) at (221, -2) {$|n\rangle$};
		\node [style=map] (27) at (222.25, -2) {$\mathcal{F}|m\rangle$};
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Zthick] (22) at (223.5, 0) {};
		\node [style=Xthick] (23) at (223.5, 1) {};
		\node [style=map, thick] (24) at (222.25, -1) {$\mathcal{F}|m\rangle$};
		\node [style=map, thick] (25) at (221, -1) {$|n\rangle$};
		\node [style=none] (28) at (223.5, -2.5) {};
		\node [style=none] (29) at (223.5, 2) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=Zthick] (22) at (223.5, 0) {};
		\node [style=Xthick] (23) at (223.5, 1) {};
		\node [style=map, thick] (24) at (222.25, -1) {$\mathcal{Z}^m$};
		\node [style=map, thick] (25) at (221, -1) {$\mathcal{X}^n$};
		\node [style=none] (28) at (223.5, -2.5) {};
		\node [style=none] (29) at (223.5, 2) {};
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		\node [style=map, thick] (22) at (223.5, 0) {$\mathcal{Z}^m$};
		\node [style=map, thick] (23) at (223.5, 1) {$\mathcal{X}^n$};
		\node [style=none] (28) at (223.5, -2.5) {};
		\node [style=none] (29) at (223.5, 2) {};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
...
Where the final part follows from the fact that $\mathcal{X}^n$ is a phase for the grey spider and $\mathcal{Z}^n$ is a phase for the white spider, followed by spider fusion



The two fragments of the ZX-calculus we have described so far are fragments of stabilizer circuits.  We proceed to overview the qudit stabilizer formalism.  The qubit stabilizer formalism was first introduced in \cite{gottesman} and later generalized to qudits in various ways.  We follow the qudit generalization provided in \cite{gota}:
...
A single qudit {\bf Weyl operator} an $d$-dimensional unitary generated by the qudit $\mathcal Z$ and $\mathcal X$ operators under tensor product and composition.
